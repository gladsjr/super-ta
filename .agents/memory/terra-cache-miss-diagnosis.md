...
- **Assinatura decisiva: miss anômalo é TOTAL (cached=0), não parcial.** Drift de prefixo (ex.: compactação) dá hit PARCIAL (13–40%); cached=0 num prompt de 70–90k com primeiros tokens estáveis = a requisição caiu numa máquina SEM cache ⇒ problema de ROTEAMENTO, não de eviction. Misses anômalos de prod: todos ≥70k de input, gaps 1–5,5 min, inclusive fora do pico (18h55).
- **O que difere prod dos probes limpos** (suspeitos restantes): `conversation` renderizada server-side (probe de Conversations foi limpo, mas rodou FORA do pico), `prompt_cache_key` compartilhada entre agentes/alunos do mesmo work (iv:workId), store:true, effort medium. Próximo discriminador: probe no pico usando Conversations API + chave compartilhada por 2 "alunos" simulados.
