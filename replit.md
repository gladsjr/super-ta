...
- **Schema** (migrations 021–037): per-submission `oral_video_key`, `oral_transcript`, `oral_eval_json`, `oral_devolutiva`, `oral_proctor_json`, `oral_voice_json`, `oral_asked_json`, plus the shared rubric columns (`grades_json`/`grade_final`/`grade_published_at`; `grades_json.penalty` holds the applied multiplier); `works.kind` + `exam_pdf`/`oral_questions` (now `{id, question, answer, weight}`) + `oral_grading_mode` (`deterministico`|`rubrica`) + `grade_penalty_json` (opt-in penalty policy, shared with the interview).
