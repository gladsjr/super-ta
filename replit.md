...
**Mock phase (current):** `lib/scenarios/mockEngine.js` fabricates the SHAPE of a run by SCRIPTED templates (zero LLM, zero tokens); persistence is JSON under `data/scenarios/` (gitignored, DB-free); there is no auth. NONE of the real cognition is wired: the `SuperOrchestratorAgent`, the Conversations API, and the existing agents are untouched. The clean boundary is the `/scenarios/api/run/*` endpoints calling `interactionStart`/`respond`/`scenarioFrame`, but the real implementation BEHIND that boundary is substantial (generalize the orchestrator to per-interaction variable context + persona turn routing + persona↔persona; redesign the Conversations structure; adapt/prep agents; migrate the store to Postgres; gate with auth). Dev-only runner without DB: `scenarios-dev.mjs` (port 5096, gitignored). No prompt-map entry yet because there are no real prompts. **Before any merge-to-main/Publish**: gate `/scenarios` behind auth and migrate the store to Postgres, or it ships unguarded.
