...
- **`OralExamExtractorAgent`** — extracts the question list from the exam material (PDF via Files API, or TXT inline); fast model. Mode-aware (`works.oral_grading_mode`): the 2nd field per question is the expected answer (deterministic) or the scoring criterion (rubric). The 2nd field stays server-side; only questions reach the student's Realtime session.
- **`OralExamEvaluatorAgent`** — compares the student's spoken transcript to the professor's material → per-question result; principal model. Schema/prompt branch by mode: deterministic → `correct/partial/incorrect` (→ 10/5/0); rubric → a 0–10 `score`. Each question is a rubric item with a weight; the exam grade is their weighted average (`lib/rubric.js#weightedFinal`), stored in `submissions.grades_json` like the interview. Seeds the default grade (editable by the professor).
- **`RubricCriterionCheckAgent`** — rubric-mode only; on saving the oral questions, advisory check of whether each scoring criterion specifies how to grade 0–10 (returns per-question warnings; never blocks). Fast model.
...
- **Schema** (migrations 021–038): per-submission `oral_video_key`, `oral_transcript`, `oral_eval_json`, `oral_devolutiva`, `oral_proctor_json`, `oral_voice_json`, `oral_asked_json`, plus the shared rubric columns (`grades_json`/`grade_final`/`grade_published_at`; `grades_json.penalty` holds the applied multiplier); `works.kind` + `exam_pdf`/`oral_questions` (`{id, question, answer, weight}`, **stable ids** — `db.setOralQuestions` preserves existing ids and assigns new ones from the monotonic `works.oral_next_qid`, never reusing a deleted id, so per-question grades stay aligned) + `oral_grading_mode` (`deterministico`|`rubrica`) + `grade_penalty_json` (opt-in penalty policy, shared with the interview). The professor's two editors (Configuração = enunciado; Avaliação = peso + 2º campo) share one in-memory question list in `oral-professor.html`.
