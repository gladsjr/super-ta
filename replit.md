...
- `routes/work.js`: Professor-facing endpoints (`/info`, `/conversation`, `/interviewer`, `/config-chat`, `/enunciado/coherence`, `/submissions/:subToken/evaluation` (+ `/student-version`, `/sections`, `/publish`), `/feedback-settings`, `/evaluations*` batch evaluate/generate/publish (serial, in-background, in-memory progress state, per-item budget check), submission management).
...
- **`InterviewEvaluatorAgent`** — professor-facing post-interview evaluator (`/w/:workToken/submissions/:subToken/evaluation`). Assesses how the student defended the work from the interviewer persona's perspective: both PDFs via `input_file`, rendered agenda, transcript serialized to text annotated with per-turn delivery metrics (latency, time-to-start-speaking, words/s, chars/s, disfluencies, written-register and polish scores — `lib/deliverySignals.js`, the same heuristics source used by the forensic `scripts/detect-ai-answers.mjs`; never audio bytes). Holistic: content decides per-question merit; delivery feeds a dedicated `delivery` section and corroborates authorship signals. When the work sets `expect_spontaneous`, delivery is elevated to a DECLARED criterion (assess whether the student answered "de cabeça"): spontaneous = conversational with natural hesitations/self-corrections; scripted = reading texture (written register + zero disfluency + steady pace), long time-to-start before polished answers, or repeated empty mic-opens (stall). Still separate axes (never lowers per-question content merit) and never auto-accuses — reports evidence for the professor to weigh. Per-turn delivery metrics come from `lib/deliverySignals.js`, which in audio mode prefers CLIENT-INSTRUMENTED timing (Phase 2): `time_to_start_ms` (question-ready → record-start) and `record_duration_ms`, sent by `student.html` with each audio turn and stored on the turn's `client_timing` in `conversation_json`; falls back to timestamp inference when absent. Output cached in `submissions.evaluation_json`; `?force=true` regenerates. NEVER exposed to the student.
- **`StudentFeedbackAgent`** — derives the student-facing FORMATIVE feedback from the internal evaluation. Strips grades/internal verdicts, authorship signals, delivery forensics and the professor's follow-up script. The PROFESSOR IS SOVEREIGN over content: by default the feedback stays on content and does not volunteer form/timing/spontaneity/authorship, but if the professor's guidelines ask to comment on an aspect present in the internal report (spontaneity, slow starts, answers that sounded prepared/read, fluency, register, any delivery dimension), the agent MUST include it — summarizing the internal eval faithfully, framed as observation ("soou preparado") calibrated for false positives, tied to the conversation. This is NOT gated by `expect_spontaneous` (that flag only drives the interviewer's opening contract and the InterviewEvaluator's internal delivery dimension — the StudentFeedbackAgent no longer receives it). Two-layer sanitization: hard prompt rules + `FORBIDDEN_PATTERNS` sweep in code (leak → retry pointing at the leak → explicit failure). `FORBIDDEN_PATTERNS` is now a single set — ACCUSATION/cause vocabulary only (autoria, plágio, colagem, IA, chatgpt/gpt/llm, fraude, suspeita): the INVIOLABLE rule no guideline relaxes is "never impute the cause / accuse" ("as respostas demoraram e soaram preparadas", never "você usou IA"). Formal accusation, if the professor wants it, is their call OUTSIDE this auto-feedback. `findForbiddenLeaks(report)` takes no flag. The `summary` is the SELF-SUFFICIENT deliverable: `derive()` receives the submission's `visibleSections` (`db.getSubmissionSections`) and, for content sections the professor chose NOT to show (`strengths`/`improvement_areas`/`study_suggestions`), returns the list EMPTY and folds its substance into the summary — so a hidden "O que pode melhorar" still surfaces the weaknesses in the prose, never vanishing. Checkboxes are thus a generation-time choice (include as faithful block vs. reinterpret into the summary); the cockpit always shows the three content toggles and hints to regenerate after changing them. Generation honors per-work professor guidelines (`works.feedback_guidelines`: tone, format, emphases — e.g. "texto corrido, sem comentários pergunta a pergunta"; hard content rules still prevail) and `per_question` is OPTIONAL (the agent uses it only where it adds value, or skips it entirely on request). Work-level settings (guidelines + `works.include_interviewer_opinion` default) live in the professor panel card "Avaliações e devolutivas" (`PATCH /w/:workToken/feedback-settings`, partial) and are applied by the batches (which propagate the opinion default to touched submissions; per-student override stays on the conversation page). TWO-STEP FLOW: generate the preview (`POST .../evaluation/student-version`, batch `POST .../evaluations/student-versions` — the only path that calls the agent) → professor reviews and optionally EDITS (`PUT .../evaluation/student-version`, shape-validated, internal-vocabulary warnings non-blocking; edited version lives in `student_evaluation_edited_json`, the auto version is preserved in `student_evaluation_json`, effective = edited ?? auto; `DELETE` discards the edit) → publish (`POST .../evaluation/publish`, batch `POST .../evaluations/publish` — visibility flag only, no LLM; `DELETE` unpublishes). Students read the effective version at `GET /s/:submissionToken/evaluation` — deliberately NOT bound to the 7-day review window. The published feedback has TWO parts: (1) the INTERVIEWER's opinion = the internal report's `interviewer_impression` published verbatim with a who-said-it disclaimer — the professor cannot edit it, only include/exclude it; (2) the professor's assessment = the formative fields (auto-filled neutral, professor-editable). **Per-section visibility**: each section (interviewer opinion, strengths, improvement areas, study suggestions) is an instant on/off toggle that the student version respects — content is always generated, visibility is separate (`PATCH .../evaluation/sections` per submission; `submissions.include_*` + `works.include_*` defaults that batches propagate; `db.getStudentEvaluation` composes the effective report by stripping off sections from the unfiltered base, and exposes `base_report` for the editor). **Per-student experimentation** (the conversation page is a cockpit): the rederive accepts an AD-HOC `{guidelines}` override (`POST .../evaluation/student-version`) that does NOT mutate the work default, so the professor tunes one student before committing; "save as work default" and "apply sections to all" (`PATCH /w/:workToken/feedback-settings` with `apply_to_all`) promote choices work-wide. The conversation page has lateral prev/next navigation (arrows + keyboard) over the work's submissions, built from `/w/:workToken/info`.
