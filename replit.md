...
- **`OralExamEvaluatorAgent`** — single model (no mode): applies each question's **rubric** to the transcript → a per-question `score` anchored at 0/2.5/5/7.5/10; principal model. Evaluate refuses questions without a rubric. Each question is a rubric item with a weight; exam grade = weighted average (`lib/rubric.js#weightedFinal`), stored in `submissions.grades_json`. Seeds the default grade (editable by the professor).
- **`RubricCriterionCheckAgent`** — on saving the oral questions, advisory check of whether each filled rubric specifies how to score (returns per-question warnings; never blocks). Fast model.
...
- **Schema** (migrations 021–039): per-submission `oral_video_key`, `oral_transcript`, `oral_eval_json`, `oral_devolutiva`, `oral_proctor_json`, `oral_voice_json`, `oral_asked_json`, plus the shared rubric columns (`grades_json`/`grade_final`/`grade_published_at`; `grades_json.penalty` holds the applied multiplier); `works.kind` + `exam_pdf`/`oral_questions` (`{id, question, answer, rubric, weight, rubric_stale}` — `answer` = optional expected answer, `rubric` = the scoring criterion (5 levels), `rubric_stale` = review flag set when question/answer changes; **stable ids** via `db.setOralQuestions` + monotonic `works.oral_next_qid`, never reusing a deleted id) + `grade_penalty_json` (opt-in penalty policy, shared with the interview). `works.oral_grading_mode` is now **vestigial** (single rubric model; kept for the 039 data migration). The professor's two editors (Configuração = pergunta + resposta; Avaliação = peso + rubrica) share one in-memory question list in `oral-professor.html`.
