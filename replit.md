...
- **`GradingAgent`** — 0–10 grade per rubric criterion (one LLM call per criterion); final grade is a weighted average computed in code (`lib/rubric.js`). Professor-only unless the `grade` section is published.

Devolutiva flow: generate preview → professor reviews/edits → publish (visibility flags). Per-section visibility (interviewer opinion, strengths, improvement areas, study suggestions, grade) is independent of generation. Students read the effective version at `GET /s/:submissionToken/evaluation`.
