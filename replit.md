...
- `lib/` — shared infra (db, session lifecycle/state, conversation utils, audio + audioStore, billing, middleware, deliverySignals, rubric, superOrchestrator/actionSchema; `oralRealtime` = Realtime relay, `proctor` = video proctoring (streaming frame extraction), `proctorQueue` = global analysis queue (lane `video_analysis` da tabela `jobs` desde o #338 — separada da retranscrição, mecânica comum; concorrência configurável, admin Operações); `units`/`rbac`/`packages` = camada institucional — árvore, RBAC tenant-aware, DSL+cotas de pacote).
...
**STT provider layer (#284)**: fala do aluno passa por `lib/stt.js#sttTranscribe` (única porta de entrada — não chame `audio.transcriptions.create` direto). Provedores: `openai` (gpt-transcribe, padrão) e `groq` (whisper-large-v3, motor DORMENTE — decisão de 21/08: Groq fora do plano, #286/#290 fechadas not-planned). Config em `policy.yaml#models.stt_*`: fallback automático por chamada, timeout (só com fallback), sombra amostral p/ comparação. O /audio do modo mensagem passa um GLOSSÁRIO por trabalho (#293, `lib/sttGlossary.js` — extração mecânica de siglas/nomes do plano+análise; a calibração NÃO recebe glossário, ela mede a captação). Provedor configurado sem credencial/preço derruba o boot (ADR 0002). Fluxos de voz: o relay grava um TEE do áudio contínuo do aluno (`lib/audioTee.js` — PCM em disco→OGG pós-sessão, fronteiras nativas, falha nunca derruba o relay) e ENFILEIRA a RETRANSCRIÇÃO de auditoria (`lib/retranscribe.js` → `submissions.final_transcript`, migration 077) na fila de jobs (`jobs`, migration 078; `lib/jobs.js` + executor `lib/jobRunner.js` — claim atômico com lease, retentativas; ADR 0022). Motor da retranscrição em `policy.yaml#jobs.retranscribe_engine`: `api` (camada #284, qualquer tique) ou `local` (faster-whisper via `scripts/transcribe_local.py`, custo zero, SÓ na janela ociosa — zero sessões de voz + memória livre mínima; exige python3 + faster-whisper na VM; falha degrada p/ API). Filhos de batch nascem com `lib/spawnLow.js` (nice 19 + 1 thread). Em CONVIVÊNCIA (corte 4): a conversa ao vivo segue sendo a fonte primária, mas os AVALIADORES (oral e entrevista realtime) recebem o bloco de auditoria como fonte de maior fidelidade da fala do aluno (`lib/auditTranscript.js`; sessão multi-parte cai p/ contínuo), o ALUNO vê a retranscrição na revisão pós-sessão, e o monitor de captação (#287) roda sobre o tee (`final_transcript.quality` — suspects=0 é o critério de "sessão íntegra" p/ a Fase 4 desligar a transcrição ao vivo, que hoje NÃO tem chave de desligar e sustenta contadores vitais do relay). O MODO MENSAGEM também retranscreve (corte 4B): no encerramento, job `retranscribe_message` refaz cada blob arquivado (`student_audio_artifacts`, com o turno exato) com o glossário do trabalho → `final_transcript` modo `answers`, consumido pelos mesmos pontos (avaliador, revisão do aluno, bloco do professor). Professor vê tudo no bloco "Retranscrição de auditoria". Modos do `final_transcript`: `segments` (fatiado por marcas), `answers` (modo mensagem), `continuous` (fallback), `too_large` e `silent` — este último quando o aluno não chegou a falar (queda logo no início, desistência antes da 1ª resposta): silêncio é REGISTRADO e o job CONCLUI, nunca vira falha retentável (#344).
