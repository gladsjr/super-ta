...
| `StudentFeedbackAgent` (deriva a devolutiva FORMATIVA ao aluno — PRÉVIA, sem publicar — em `/w/:workToken/submissions/:subToken/evaluation/student-version` e no lote `/w/:workToken/evaluations/student-versions`; publicar é passo separado em `/evaluation/publish`) | [agents/StudentFeedbackAgent.js](../agents/StudentFeedbackAgent.js#L112) | preâmbulo (`student_via_ui`) + `systemPromptBody` + diretrizes do professor (`works.feedback_guidelines` — tom/formato/ênfases; `per_question` opcional) + bloco de exceção quando `works.expect_spontaneous` (libera comentar tempo/leitura/"de cabeça") + relatório interno como input; saída sanitizada por `FORBIDDEN_PATTERNS` no código (split ALWAYS vs relaxável-por-espontaneidade; vazou → retry → falha) |
...
flowchart LR
  ...
  click FeedbackAgent "vscode://file/c:/Users/glads/src/super-ta/agents/StudentFeedbackAgent.js:112" "Abre o systemPromptBody do StudentFeedbackAgent"
```
