...
| `InterviewEvaluatorAgent` (avalia a entrevista sob a perspectiva do entrevistador, em `/w/:workToken/submissions/:subToken/evaluation`) | [agents/InterviewEvaluatorAgent.js](../agents/InterviewEvaluatorAgent.js#L167) | preâmbulo (`professor_via_ui`) + `systemPromptBody` (inclui `EXTEMPORANEOUS_ANSWER_PRINCIPLE`) + agenda + transcrição serializada (conteúdo apenas — métricas de forma/entrega removidas em 12/08/2026, ver #282); PDFs (enunciado + entrega) via `input_file` |
| `StudentFeedbackAgent` (deriva a devolutiva FORMATIVA ao aluno — PRÉVIA, sem publicar — em `/w/:workToken/submissions/:subToken/evaluation/student-version` e no lote `/w/:workToken/evaluations/student-versions`; publicar é passo separado em `/evaluation/publish`) | [agents/StudentFeedbackAgent.js](../agents/StudentFeedbackAgent.js#L104) | preâmbulo (`student_via_ui`) + corpo em DUAS variantes (`buildPromptBody`): COM summary gerado (prova oral, cenários) e SEM summary (#274, entrevistas — o "Comentário do professor" é campo aberto escrito pelo próprio professor; a devolutiva gerada são per_question + listas) + diretrizes do professor (`works.feedback_guidelines` — tom/formato/ênfases E QUAIS ASPECTOS entram; professor soberano sobre o conteúdo: se pedir, comenta tempo/leitura/"de cabeça") + relatório interno como input; saída sanitizada por `FORBIDDEN_PATTERNS` no código (conjunto único de ACUSAÇÃO; regra inviolável = não imputar causa; vazou → retry → falha). NÃO depende de `works.expect_spontaneous` |
...
flowchart LR
  ...
  click FeedbackAgent "vscode://file/c:/Users/glads/src/super-ta/agents/StudentFeedbackAgent.js:104" "Abre o buildPromptBody do StudentFeedbackAgent (duas variantes: com/sem summary)"
...
2. **`systemPromptBody` + preâmbulo padronizado em classes de agente** ([agents/](../agents/)):
   ...
   **Conjunto ativo após a reforma do super-orquestrador:**
   ...
   - **Prova oral (Realtime)** — subsistema à parte (`kind='oral_realtime'`), desenhado no diagrama dedicado em *Prova oral (Realtime) — relay e prompts* (abaixo):
     ...
     - Instruções do examinador (persona fixa + protocolo de condução + perguntas) — string montada em [lib/oralRealtime.js](../lib/oralRealtime.js)#buildExamInstructions, enviada como `instructions` da sessão Realtime (fala-a-fala). O gabarito NUNCA entra na sessão; só as perguntas. Cobertura: o examinador só aponta lacuna a partir do ENUNCIADO e só quando ele deixa óbvio (ex.: pediu N itens e o aluno deu menos) — nunca revela nem corrige. O motor de relay (bridge de áudio, VAD, vigia de silêncio, encerramento com despedida por TTS, keep-alive, metering) é genérico e vive em [lib/realtimeBridge.js](../lib/realtimeBridge.js); `oralRealtime.js` é o adaptador da prova oral.
   - **Entrevista SIMPLIFICADA (tempo real)** — variante da entrevista (`kind='interview'` + `works.interview_variant='realtime'`, migration 050): mesma configuração do professor e mesmo pipeline de avaliação, mas a conversa roda numa sessão Realtime fala-a-fala (telas do aluno estilo prova oral, em [static/live-student.html](../static/live-student.html)):
     - Preparação: no upload do aluno (`POST /s/:t/live/upload`, [routes/interviewLive.js](../routes/interviewLive.js)), o **PrepBuilderAgent** (o MESMO da entrevista por mensagens) gera análise + plano; persistidos em `submissions.live_prep_json`.
     - Instruções do entrevistador — string montada em [lib/liveInterview.js](../lib/liveInterview.js)#buildLiveInterviewInstructions: agenda da persona (renderInterviewerAgenda do `interviewer_yaml`) + protocolo de condução por voz (herdado do da prova oral) + SÓ as perguntas do plano (`rationale`/`concerns` NUNCA entram — seriam dicas). Retomada em `buildLiveResumeInstructions`. Enviada como `instructions` da sessão Realtime via o motor genérico [lib/realtimeBridge.js](../lib/realtimeBridge.js) (relay em `/s/:t/live/relay`).
     - Pós-conversa: o transcript vira `conversation_json` no formato da entrevista ([lib/liveConversation.js](../lib/liveConversation.js), marcado com `channel:'realtime_voice'`) → avaliação/devolutiva/nota/publicação usam o pipeline EXISTENTE (`InterviewEvaluatorAgent` etc.) sem mudanças; o vídeo reusa `oral_video_parts` + proctoring da entrevista.
   - **Penalidade automática por alertas — REMOVIDA (13/08/2026).** O `GradePenaltyAgent` foi apagado: o proctoring é indício para REVISÃO HUMANA, nunca acusação automática. Hoje os alertas de vídeo só são MOSTRADOS ao professor (que ajusta a nota à mão, se quiser) e podem colorir a devolutiva do aluno. A nota é só a média ponderada da rubrica. Sobrou de [lib/gradePenalty.js](../lib/gradePenalty.js) apenas `renderProctorForStudent` (observações neutras para a devolutiva). A coluna `works.grade_penalty_json` ficou órfã. O `InterviewEvaluatorAgent` também não emite mais `delivery`/`authorship_signals`/`authorship_confidence` (eram para inferir voz; o vídeo cobre a integridade).
