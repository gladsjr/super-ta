...
- **Professor side** (`static/conversation.html`): a **"Fiscalização por vídeo"** card runs analysis on demand (`POST /w/:t/submissions/:sub/proctor` → `analyzeOralVideoParts` → `oral_proctor_json`), plays the video (`GET .../proctor-video/:idx`, Range/seek — served by `lib/serveMedia.js` with **true partial reads**; see "Video delivery" below), and shows alert pills. The report **composes the grade** via the shared penalty (`lib/gradePenalty.js#applyPenaltyToGrades` flow `"interview"` now combines authorship + `renderOralAlerts`) and can surface as a soft devolutiva note. Proctoring is deliberately NOT fed into the `InterviewEvaluatorAgent` reasoning (keeps integrity separate from content).
...
`lib/serveMedia.js` streams only the requested span, through
