 ...
 *               disfluencies, recording: {duration_s, wps, thinking_s}|null,
 *               flags: [string] }
 */
export function buildDeliveryReport(conversation, audioArtifacts = []) {
    ...
    const turns = (Array.isArray(conversation?.turns) ? conversation.turns : [])
        ...
        .map(t => {
            ...
            let recording = null;
            if (mode === "audio") {
                const ct = t.client_timing && typeof t.client_timing === "object" ? t.client_timing : null;
                const recs = byTurn.get(t.index) || [];
                const finalRec = recs.length > 0 ? recs[recs.length - 1] : null;
                // Duração da fala: preferir a medida do cliente (precisa) sobre a
                // duração do artefato. Tempo até começar a falar: do cliente
                // (instrumentado na Fase 2 — preciso) quando houver; senão a
                // aproximação por timestamps (inclui a escuta da pergunta).
                const ctDurS = ct && num(ct.record_duration_ms) != null ? num(ct.record_duration_ms) / 1000 : null;
                const artifactDur = finalRec ? num(finalRec.duration_s) : null;
                const dur = ctDurS ?? artifactDur;
                let timeToStartS = null, startSource = null;
                if (ct && num(ct.time_to_start_ms) != null) {
                    timeToStartS = Math.max(0, Math.round(num(ct.time_to_start_ms) / 1000));
                    startSource = "client";
                } else if (dur && dur > 0) {
                    const ivs = Array.isArray(t.interventions) ? t.interventions : [];
                    const ref = ivs.length > 0 ? (ivs[ivs.length - 1].at ?? t.asked_at) : t.asked_at;
                    const total = secondsBetween(ref, t.answered_at);
                    timeToStartS = total != null ? Math.max(0, Math.round(total - dur)) : null;
                    startSource = "inferido";
                }
                if (dur && dur > 0) {
                    const wps = w / dur;
                    recording = {
                ...
                } else if (timeToStartS != null) {
                    recording = { duration_s: null, wps: null, time_to_start_s: timeToStartS, start_source: startSource };
                }
                // Anti-gaming: abriu o microfone (gravação não-trivial) mas quase
                // não disse nada — possível "começar pra ganhar tempo".
                if (ct && num(ct.record_duration_ms) != null && num(ct.record_duration_ms) >= TH.STALL_OPEN_MS && w < TH.LOW_CONTENT_WORDS) {
                    flags.push(`gravou ${(num(ct.record_duration_ms) / 1000).toFixed(1)}s mas disse quase nada (${w} palavra(s)) — possível abrir o microfone só para "começar"`);
                }
                ...
                if (latency != null && latency >= TH.LONG_LATENCY_S && w >= TH.LONG_ANSWER_WORDS) {
                    flags.push(`${Math.round(latency)}s entre a pergunta e a resposta final longa — tempo compatível com gerar fora e ler`);
                }
            }
