    ...
    const turns = (Array.isArray(conversation?.turns) ? conversation.turns : [])
        ...
        .map(t => {
            ...
            if (mode === "audio") {
                ...
                const dur = finalRec ? num(finalRec.duration_s) : null;
                if (dur && dur > 0) {
                    const wps = w / dur;
                    // Referência do "tempo até começar a falar": a última fala do
                    // entrevistador no turno (follow-up) ou a pergunta original.
                    // Em modo áudio inclui o tempo OUVINDO a pergunta via TTS —
                    // é aproximação, não cronômetro.
                    const ivs = Array.isArray(t.interventions) ? t.interventions : [];
                    const ref = ivs.length > 0 ? (ivs[ivs.length - 1].at ?? t.asked_at) : t.asked_at;
                    const total = secondsBetween(ref, t.answered_at);
                    const thinking = total != null ? Math.max(0, Math.round(total - dur)) : null;
                    recording = { duration_s: Math.round(dur), wps: Number(wps.toFixed(2)), thinking_s: thinking };
