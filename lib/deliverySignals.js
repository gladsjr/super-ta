...
export const TH = {
    ...
    STALL_OPEN_MS: 3000,
    LOW_CONTENT_WORDS: 5,
    // ECO DE DICA: fração do vocabulário NOVO da resposta final (não presente na
    // pergunta original) que veio do follow-up do PRÓPRIO entrevistador. Alto =
    // resposta pós-dica que devolve a formulação do entrevistador — proveniência
    // do conteúdo é dele, não do aluno. Calibrado com o caso real "Mackenzie"
    // (eco flagrante ~0.6; resposta própria do mesmo aluno ~0.1).
    ECHO_MIN: 0.4,
    ECHO_MIN_WORDS: 10, // só vale o sinal em respostas com conteúdo (não "sim, exato")
};
...
// ---------------------------------------------------------------------
// ECO DE DICA — proveniência do conteúdo da resposta
// ---------------------------------------------------------------------
// Stopwords PT (sem acentos — os tokens também são normalizados sem acento).
const STOPWORDS_PT = new Set(
    ("a o e e de da do das dos em no na nos nas um uma uns umas que com por para pra sem sob sobre entre ate como " +
     "mais menos muito muita muitos muitas pouco pouca ser estar foi era sao esta estao tem ter ha voce voces eu ele " +
     "ela eles elas nos isso isto aquilo esse essa esses essas este esta estes estas seu sua seus suas meu minha meus " +
     "minhas dele dela quando onde porque porque entao tambem ja nao sim mas ou se ao aos as pelo pela pelos pelas lhe " +
     "lhes me te vos qual quais quem cujo cuja outro outra outros outras mesmo mesma mesmos mesmas proprio propria tal " +
     "tais cada algum alguma alguns algumas nenhum nenhuma todo toda todos todas vez vezes coisa coisas gente agora " +
     "aqui ali la bem mal so ainda depois antes durante fosse seria seriam fica ficam ficar vai vao vou ia iam pode " +
     "podem poderia queria quero acho acredito exatamente obviamente claro enfim tipo assim ne").split(/\s+/)
);
function stripAccents(s) { return String(s).normalize("NFD").replace(/[̀-ͯ]/g, ""); }
// Tokens de CONTEÚDO únicos: minúsculos, sem acento, sem stopword, len>=3.
...
// Um token "casa" com o conjunto se é igual OU se um contém o outro (>=5 chars:
// pega flexões tipo "montado"~"remontado", "reproduzido"~"reproduzidas").
function tokenInSet(tok, arr) {
    for (const h of arr) {
        if (tok === h) return true;
        if (tok.length >= 5 && h.length >= 5 && (h.includes(tok) || tok.includes(h))) return true;
    }
    return false;
}
/**
 * Eco de dica (0..1): do vocabulário NOVO da resposta (tokens de conteúdo que
 * NÃO estavam na pergunta original), que fração veio da fala do entrevistador
 * no follow-up. Alto = a resposta devolve a formulação do entrevistador (o
 * conteúdo foi ditado pela dica). null quando não há base de comparação.
 */
export function echoScore(hintText, answerText, questionText = "") {
    const hintArr = contentTokens(hintText);
    if (!hintArr.length) return null;
    const qArr = contentTokens(questionText);
    const novel = contentTokens(answerText).filter(tok => !tokenInSet(tok, qArr));
    if (novel.length < 4) return null; // base pequena demais para um percentual honesto
    const hits = novel.filter(tok => tokenInSet(tok, hintArr)).length;
    return Number((hits / novel.length).toFixed(2));
}
...
export function buildDeliveryReport(conversation, audioArtifacts = []) {
    ...
    const turns = (Array.isArray(conversation?.turns) ? conversation.turns : [])
        ...
        .map(t => {
            ...
            // ECO DE DICA (independe do modo): compara a resposta FINAL com o
            // último follow-up do entrevistador no turno. Só existe quando houve
            // follow-up — resposta direta à pergunta não tem dica para ecoar.
            let echo = null;
            {
                const ivs = Array.isArray(t.interventions) ? t.interventions : [];
                const lastHint = [...ivs].reverse().find(iv => iv && iv.type !== "meta" && iv.assistant_response);
                if (lastHint) {
                    echo = echoScore(lastHint.assistant_response, answer, t.question || "");
                    if (echo != null && echo >= TH.ECHO_MIN && w >= TH.ECHO_MIN_WORDS) {
                        flags.push(`ECO DE DICA: ${Math.round(echo * 100)}% do vocabulário novo da resposta final veio do próprio follow-up do entrevistador — resposta pós-dica devolvendo a formulação dele, não demonstração de domínio próprio`);
                    }
                }
            }
