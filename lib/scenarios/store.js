...
export async function createRun(scenarioId, submissionId = null) {
    ...
    return { id, scenario_id: scenarioId, submission_id: submissionId, mode: "mock", interaction_index: 0, transcript: [], memory: null, evaluation_json: null, grades_json: null, done: false, student_work: {} };
}
export async function saveRun(run) {
    await pool.query(
        `UPDATE scenario_runs SET mode = $2, interaction_index = $3, transcript = $4, memory = $5,
            evaluation_json = $6, grades_json = $7, done = $8, student_work_json = $9, updated_at = now() WHERE id = $1`,
