...
export function renderInteractionBriefing({ scenario, interaction, personasById, position, total, runMemory, enunciadoAvailable = false, studentWorkAvailable = false }) {
    ...
    for (const pi of (it.private_info || [])) {
        for (const pid of (pi.persona_ids || [])) (privateByPersona[pid] = privateByPersona[pid] || []).push(str(pi.text));
    }
