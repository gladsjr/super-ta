...
export const SCENARIO_TURN_SCHEMA_DESCRIPTION = `
{
  ...
  "action": {
    ...
    "hint": { "title": "rótulo curto", "body": "1-2 frases" }
    // OPCIONAL e raro. Um BALÃO DE DICA mostrado ao aluno FORA do role-play (não é fala de persona).
    // Use SÓ quando a outra ponta levou a conversa a um tópico FORA DO ESCOPO do cenário: a persona
    // responde de forma evasiva/redireciona em "message" (sem revelar que é "fora do escopo"), e o
    // "hint" avisa o aluno, em meta, que aquele tópico está fora do escopo. Omita o campo quando não se aplica.
  },
...
export function validateScenarioTurn(parsed, allowedIds = []) {
    ...
    // speaker_persona_id só é exigido em "speak" (advance/finalize não precisam).
    // O agente normaliza nome→id antes de validar, então aqui ele já deve ser válido.
    if (a.kind === "speak") {
        if (!isNonEmptyStr(parsed.speaker_persona_id)) errors.push("speaker_persona_id é obrigatório em speak");
        else if (allow.size && !allow.has(parsed.speaker_persona_id)) errors.push(`speaker_persona_id fora dos participantes: ${parsed.speaker_persona_id}`);
    }
    // hint é opcional; quando presente, precisa de um body com texto (title é opcional).
    if (a.hint !== undefined && a.hint !== null) {
        if (typeof a.hint !== "object" || !isNonEmptyStr(a.hint.body)) errors.push("action.hint, se presente, precisa de um body não-vazio");
    }
