...
export async function prepAndOpenLive(prepAgent, agent, { scenario, interaction, personasById, idx, total, runMemory = "", meterCtx = null, interactionMode = "text", enunciadoFileId = null, studentWorkFileId = null }) {
    const position = idx + 1;
    if (interaction.kind === "persona_exchange") {
        const ex = await agent.personaExchange({ scenario, interaction, personasById, position, total, runMemory, interactionMode, meterCtx });
        const ids = (interaction.participants || []).map(p => p.persona_id);
    ...
    const plan = await prepAgent.prepInteraction({ scenario, interaction, personasById, position, total, runMemory, enunciadoFileId, studentWorkFileId, meterCtx });
...
export async function respondLive(agent, { scenario, interaction, personasById, idx, total, transcript, memory = null, meterCtx = null, interactionMode = "text", studentName = null, studentGenderHint = null, vectorStoreId = null, studentWorkVectorStoreId = null, timeState = null, onFirstDelta = null, onMessageReady = null }) {
    ...
    const out = await agent.studentTurn({
        ...
        vectorStoreId, studentWorkVectorStoreId, timeState, interactionMode, studentName, studentGenderHint, meterCtx,
