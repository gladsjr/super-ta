...
// e a memória de run vivem AQUI, no código, não no LLM. A navegação entre
// interações continua nas rotas/harness. NÃO há teto de turnos por interação:
// a trava do aluno é o TEMPO da etapa (time_limit_min), fiel à natureza da
// conversa — decisão de produto (era um cap de 8 que silenciava a persona).

import { entry, interactionHeader, scenarioFrame, ROLE_LABEL } from "./mockEngine.js";
import { formLabel, normalizeForm } from "./interactionForms.js";

export { scenarioFrame, interactionHeader }; // reexport: idênticos ao mock (sem LLM)
...
export function buildRunMemory(scenario, transcript, uptoIdx) {
    ...
    for (let i = 0; i < uptoIdx; i++) {
        ...
        lines.push(`- "${it.title}" (${formLabel(normalizeForm(it))}): ${parts.join(". ") || "sem falas registradas"}`);
    }
    return lines.join("\n");
}
...
export async function interactionStartLive(agent, { scenario, interaction, personasById, idx, total, runMemory = "", meterCtx = null, interactionMode = "text", vectorStoreId = null, studentWorkVectorStoreId = null }) {
    ...
    const op = await agent.interactionOpener({ scenario, interaction, personasById, position, total, runMemory, interactionMode, meterCtx, vectorStoreId, studentWorkVectorStoreId });
...
// Prep + abertura de uma interação (fluxo do aluno). Roda o PREP (lê o contexto
// inteiro — enunciado/trabalho do aluno por inteiro) e devolve a abertura da
// persona + o PLANO (que semeia run.memory). Para `persona_exchange`, mantém a
// troca atual (sem prep). O CABEÇALHO é separado (interactionHeader), empurrado
// quando a etapa fica ativa; a abertura vem aqui, quando a etapa fica PRONTA
// (sem trabalho, ou já anexado).
export async function prepAndOpenLive(prepAgent, agent, { scenario, interaction, personasById, idx, total, runMemory = "", meterCtx = null, interactionMode = "text", enunciadoFileId = null, studentWorkFileId = null, artifactFileId = null }) {
    const position = idx + 1;
    if (interaction.kind === "persona_exchange") {
        const ex = await agent.personaExchange({ scenario, interaction, personasById, position, total, runMemory, interactionMode, meterCtx });
        const ids = (interaction.participants || []).map(p => p.persona_id);
        ...
        return { entries: asides, memory: null };
    }
    const plan = await prepAgent.prepInteraction({ scenario, interaction, personasById, position, total, runMemory, enunciadoFileId, studentWorkFileId, artifactFileId, meterCtx });
    const opener = personasById[interaction.opener_persona_id] || personasById[(interaction.participants || [])[0]?.persona_id];
    return { entries: [entry(opener, "persona", plan.opening, { role: roleOf(interaction, opener?.id) })], memory: plan };
}
...
export async function respondLive(agent, { scenario, interaction, personasById, idx, total, transcript, memory = null, meterCtx = null, interactionMode = "text", studentName = null, studentGenderHint = null, vectorStoreId = null, studentWorkVectorStoreId = null, artifactVectorStoreId = null, privateArtifactVectorStoreIds = [], timeState = null, onFirstDelta = null, onMessageReady = null, onMessageDelta = null, onSpeaker = null }) {
    if (interaction.kind !== "student") return { entries: [], action: { kind: "advance" }, memory };

    // Segmento da etapa `idx` — com o RETORNO às conversas anteriores, o turno pode
    // mirar uma etapa que não é a última; o chamador já inseriu a fala do aluno lá.
    const segs = splitByInteraction(transcript);
    const seg = (Number.isInteger(idx) && segs[idx]) ? segs[idx] : (segs.pop() || []);
    ...
    const out = await agent.studentTurn({
        ...
        vectorStoreId, studentWorkVectorStoreId, artifactVectorStoreId, privateArtifactVectorStoreIds, timeState, interactionMode, studentName, studentGenderHint, meterCtx,
        onFirstDelta, onMessageReady, onMessageDelta, onSpeaker,
    });
    ...
    // Balão de DICA (fora do role-play): a persona já respondeu evasiva em message;
    // o hint avisa o aluno, p.ex., que o tópico está fora do escopo do cenário.
    const hint = out.action?.hint;
    if (hint && typeof hint === "object" && String(hint.body || "").trim()) {
        entries.push(entry(null, "hint", String(hint.body).trim(), { title: String(hint.title || "Dica").trim() }));
    }
