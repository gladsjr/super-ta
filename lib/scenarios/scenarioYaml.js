...
export function scenarioToYaml(scenario) {
    ...
    const doc = {
        ...
        personas: (s.personas || []).map(personaToObj),
        interactions: (s.interactions || []).map((it) => {
            const isEx = it.kind === "persona_exchange";
            const o = { title: str(it.title), kind: it.kind || "student", objective: str(it.objective_type) };
...
function scenarioFromObj(data) {
    ...
    const interactions = (Array.isArray(data.interactions) ? data.interactions : []).map((it, i) => {
        const kind = it.kind === "persona_exchange" ? "persona_exchange" : "student";
        ...
        return {
            title: str(it.title),
            kind,
            objective_type: str(it.objective ?? it.objective_type),
            participants,
    ...
    return { name: str(data.name), description: str(data.description), personas, interactions };
}
