...
// não consegue isolar o benchmark da produção (mesmo projeto).
...
async function fetchUsageBuckets(endpoint, { startTime, endTime, bucketWidth = "1d", groupBy = ["model"], apiKeyIds = null }) {
...
function normalizeUsageResult(category, res) {
    return {
        ...
        input_tokens: Number(res.input_tokens ?? 0),
        cached_tokens: Number(res.input_cached_tokens ?? res.cached_tokens ?? 0),
...
export async function fetchBenchmarkUsage({ startTime, endTime, bucketWidth = "1d" }) {
    ...
    if (!keyId) notes.push("BENCHMARK_OPENAI_API_KEY_ID ausente — Usage NÃO filtrada por chave (inclui produção do projeto)");
    return { lines, notes };
}
...
export async function fetchProjectCosts({ startTime, endTime }) {
    ...
    return { total_usd: total, by_line_item: byLineItem };
}
...
export function referenceUsdForUsage(line, pricing) {
    ...
    if (line.category === "completions") {
        ...
        const billableInput = Math.max(0, line.input_tokens - line.cached_tokens);
        const usd = (billableInput * e.input_per_mtok + line.cached_tokens * e.input_cached_per_mtok + line.output_tokens * e.output_per_mtok) / 1e6;
