...
import { OralExamExtractorAgent } from "../agents/OralExamExtractorAgent.js";
import { OralExamEvaluatorAgent } from "../agents/OralExamEvaluatorAgent.js";
import { GradePenaltyAgent } from "../agents/GradePenaltyAgent.js";
import { OralRubricBuilderAgent } from "../agents/OralRubricBuilderAgent.js";
import { OralCalibrationAgent } from "../agents/OralCalibrationAgent.js";
...
// Checagem MÍNIMA de persona do enunciado (issue #133): roda no upload, barata → fast_model.
export const enunciadoCoherenceAgent = new EnunciadoCoherenceAgent(openai, FAST_MODEL);
...
// Variantes RÁPIDAS (modelo barato): só para o "Testar (rápido)" do professor —
// submissões de teste, para iterar o desenho do cenário sem gastar o gpt-5.5.
export const scenarioOrchestratorAgentFast = new ScenarioOrchestratorAgent(openai, FAST_MODEL);
export const scenarioPrepAgentFast = new ScenarioPrepAgent(openai, FAST_MODEL);
// Assistente do professor no estúdio (JSON de propostas). Usa o FAST_MODEL:
// o gargalo do estúdio é a LATÊNCIA percebida (edição pequena deve responder
// em ~2s; o gpt-5.5 leva ~5s só raciocinando antes do 1º token). O gpt-5.4-mini
// atinge ~2s em edições mecânicas E preserva o desenho de informação (arco de
// descoberta, info privada, entrevistar o professor) — verificado em smoke.
// Resolve a issue #36 e barateia ~10–40×.
...
// Prova oral (Realtime): extrai perguntas+respostas do PDF da prova (modelo rápido).
export const oralExamExtractorAgent = new OralExamExtractorAgent(openai, FAST_MODEL);
// Prova oral: avalia comparando a transcrição do aluno ao material do professor (modelo forte).
export const oralExamEvaluatorAgent = new OralExamEvaluatorAgent(openai, PRINCIPAL_REASONING_MODEL);
// Critério fixo de penalidade por alertas (proctoring na oral / autoria na entrevista):
// lê os alertas + política do professor e devolve um multiplicador aplicado à nota. Modelo forte.
export const gradePenaltyAgent = new GradePenaltyAgent(openai, PRINCIPAL_REASONING_MODEL);
// Prova oral: gera a rubrica detalhada (5 níveis) + peso a partir da resposta esperada.
// Modelo forte (a qualidade da rubrica governa a nota).
export const oralRubricBuilderAgent = new OralRubricBuilderAgent(openai, PRINCIPAL_REASONING_MODEL);
// Prova oral: gera a frase de calibração de fala (pré-teste de captação) + termos-chave
// a partir das perguntas/respostas. Modelo rápido (é uma frase curta, não raciocínio pesado).
export const oralCalibrationAgent = new OralCalibrationAgent(openai, FAST_MODEL);
