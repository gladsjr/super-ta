...
// lib/healthDeep.js e entra no mesmo registro; o `e2e` é recusado com 501
// até o corte 4.
//
...
// Que níveis cada profundidade executa: deep inclui shallow (o relatório
// pós-Publish é um só). e2e fica para o corte 4.
const LEVELS_BY_DEPTH = { shallow: ["shallow"], deep: ["shallow", "deep"] };
...
export async function runHealth({ ids = null, depth = "shallow" } = {}) {
    ...
    if (!niveis) {
        throw Object.assign(new Error(`nível "${depth}" ainda não implementado — só ${Object.keys(LEVELS_BY_DEPTH).join(" e ")}`), { httpStatus: 501 });
    }
    if (ids) {
        ...
        if (acima.length) throw Object.assign(new Error(`checks de nível acima de depth=${depth}: ${acima.join(", ")} — use depth=deep`), { httpStatus: 400 });
    }
