...
export async function comPrazo(promise, ms, rotulo = "prazo") {
    let timer;
    const prazo = new Promise((_, rej) => { timer = setTimeout(() => rej(new Error(`${rotulo} de ${ms} ms estourado`)), ms); });
...
async function adquirirCliente(ms) {
    return comPrazo(healthPool.connect(), ms, "conexão com o banco");
}
...
async function checkSchema(ctx) {
    ...
    const missing = diffExpected(EXPECTED_SCHEMA, catalog);
...
function versaoFfmpeg() {
    if (ffmpegVisto) return Promise.resolve(ffmpegVisto);
    return new Promise((resolve) => {
        try {
            execFile("ffmpeg", ["-version"], { encoding: "utf8", timeout: 2000 }, (err, stdout) => {
                if (err) return resolve(null);
                ffmpegVisto = (String(stdout).split("\n")[0] || "").replace(/^ffmpeg version\s+/, "").split(" ")[0] || null;
                resolve(ffmpegVisto);
            });
        } catch { resolve(null); }
    });
...
async function checkAssets() {
    ...
    const ffmpeg = await versaoFfmpeg();
