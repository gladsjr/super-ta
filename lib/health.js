...
//                seguinte)
// Este arquivo implementa o nível `shallow`; o `deep` vive em
// lib/healthDeep.js e o `e2e` em lib/healthE2e.js, no mesmo registro.
//
...
import { DEEP_CHECKS, estimateDeepCostUsd, healthBudgetStatus } from "./healthDeep.js";
import { E2E_CHECKS } from "./healthE2e.js";
...
// Irmão de ESCRITA do comClienteRO, para a sonda e2e (#375, corte 4b): cria e
// apaga envios de teste e lança a estimativa de custo. Mesma disciplina —
// pool do health (o prazo de aquisição tira o pedido da fila), transação com
// statement_timeout (quem cancela é o servidor), prazo estourado descarta o
// cliente. Nunca uma operação da sonda fica pendurada no pool do app depois
// de o relatório voltar (revisão do #399).
export async function comClienteRW(fn, ms = SHALLOW_TIMEOUT_MS) {
    ...
    try {
        return await comPrazo((async () => {
            ...
            try {
                await client.query(`SET LOCAL statement_timeout = ${Math.max(500, ms - 500)}`);
                const out = await fn((sql, values) => client.query(sql, values));
                await client.query("COMMIT");
                return out;
            } catch (err) {
...
export const CHECKS = [
    ...
    // Nível deep (#375, corte 3): uma ida real a cada dependência externa.
    ...DEEP_CHECKS,
    // Nível e2e (#375, corte 4b): o relay de voz completo, sob pedido explícito.
    ...E2E_CHECKS,
];
export const CHECK_IDS = CHECKS.map(c => c.id);
// Que níveis cada profundidade executa: cada nível inclui os anteriores (o
// relatório pós-Publish é um só).
const LEVELS_BY_DEPTH = { shallow: ["shallow"], deep: ["shallow", "deep"], e2e: ["shallow", "deep", "e2e"] };
export { estimateDeepCostUsd };
...
const montar = (check, status, t0, detail, cost_usd = 0) =>
    ({ id: check.id, label: check.label, level: check.level, status, duration_ms: Date.now() - t0, detail, cost_usd: Number(cost_usd) || 0 });
...
async function executarUm(check, ctx) {
    ...
    // Prazo estourado avisa o corpo abandonado pelo signal: quem abriu socket
    // ou processo fecha o que abriu (revisão do #394 — uma sonda do Realtime
    // silenciosa ficava viva depois do relatório voltar).
    const ac = new AbortController();
    try {
        const corpo = check.db
            ? emTransacaoRO(ctx.client, (q) => check.run({ ...ctx, q, signal: ac.signal }))
            : check.run({ ...ctx, signal: ac.signal });
        const r = await comPrazo(corpo, check.budget_ms ?? SHALLOW_TIMEOUT_MS, `check ${check.id}`);
        return montar(check, r.status, t0, r.detail ?? {}, r.cost_usd);
    } catch (err) {
        log.warn("HEALTH", `check ${check.id} falhou: ${err.message}`);
        if (err.timeout) ac.abort();
...
export async function runHealth({ ids = null, depth = "shallow" } = {}) {
    ...
    const niveis = LEVELS_BY_DEPTH[depth];
    if (!niveis) {
        throw Object.assign(new Error(`nível "${depth}" não implementado — só ${Object.keys(LEVELS_BY_DEPTH).join(", ")}`), { httpStatus: 501 });
    }
    if (ids) {
        ...
        // Pedir um check de nível acima da profundidade não é "pular": é 400,
        // porque um relatório sem ele diria menos do que o robô pediu.
        const acima = ids.filter(i => !niveis.includes(CHECKS.find(c => c.id === i).level));
        if (acima.length) throw Object.assign(new Error(`checks de nível acima de depth=${depth}: ${acima.join(", ")} — use depth=${CHECKS.find(c => c.id === acima[0]).level}`), { httpStatus: 400 });
    }
    const temPagos = CHECKS.some(c => c.paid && niveis.includes(c.level) && (!ids || ids.includes(c.id)));
    // Orçamento é dependência obrigatória de checks pagos, mesmo com filtro.
    const escolhidos = CHECKS.filter(c => niveis.includes(c.level) && (!ids || ids.includes(c.id) || (temPagos && c.id === "budget")));
    const budget = escolhidos.find(c => c.id === "budget");
    const t0 = Date.now();

    // `bancoLivre` resolve quando a sequência de banco DEVOLVEU o cliente: os
    // checks externos que precisam de uma leitura (realtime_a, lista de vozes)
    // esperam por ele e usam `leituraRO` — pelo pool do health, com prazo —
    // nunca o pool do app sem prazo (revisão do #394).
    let liberarBanco;
    const ctx = { client: null, erroBanco: null, acquire_ms: 0, deps: {}, leituraRO: (fn) => comClienteRO(fn), escritaComPrazo: (fn, ms) => comClienteRW(fn, ms), bancoLivre: new Promise(r => { liberarBanco = r; }) };
    async function lerOrcamento() {
        try { ctx.orcamento = await comClienteRO((q) => healthBudgetStatus(q)); }
        catch (err) { log.warn("HEALTH", `orçamento de saúde não resolvido: ${err.message}`); ctx.orcamento = null; }
    }
    const deBanco = escolhidos.filter(c => c.db);
    if (deBanco.length) {
    ...
    try {
        // No shallow, os checks sem banco (config, assets) correm em paralelo com
        // a sequência de banco — são triviais. No deep, os externos (spawn de
        // python/ffmpeg, provedores) esperam a sequência de banco terminar:
        // medido em prod (08/09), o `db` deu warn com 2,5 s durante os spawns
        // do próprio deep — o número do banco tem de ser do banco, não da
        // carga que o relatório gerou. Custa ~2 s a mais por execução deep.
        const semBanco = (async () => {
            if (depth !== "shallow") {
                await ctx.bancoLivre;
                if (budget) await lerOrcamento();
            }
            const podeGastar = ctx.orcamento?.work_id && !ctx.orcamento.exceeded;
            const out = await Promise.all(escolhidos.filter(c => !c.db && c.id !== "budget").map(c => executarUm(c, ctx)));
            // O ledger já recebeu os custos: o relatório deve refletir esta execução.
            // Se a leitura final falhar, não reutiliza um saldo antigo como saudável.
            if (temPagos && podeGastar) await lerOrcamento();
            if (budget) out.push(await executarUm(budget, ctx));
            return out;
        })();
        const comBanco = (async () => {
            const out = [];
            try {
                for (const c of deBanco) out.push(await executarUm(c, ctx));
            } finally {
                // O cliente volta ao pool AQUI, assim que o último check de banco
                // termina — não no fim do relatório. Um deep esperando o
                // Responses por 5 s não pode segurar a conexão única do health e
                // dar 503 falso à monitoração shallow concorrente (revisão do #394).
                ctx.client?.release();
                ctx.client = null;
                liberarBanco();
            }
        ...
        return {
            ...
            cost_usd: Math.round(checks.reduce((a, c) => a + (c.cost_usd || 0), 0) * 1e6) / 1e6,
            checks,
        };
    } finally {
        ctx.client?.release(); // só se a sequência de banco não chegou a rodar
        liberarBanco();
    }
}
