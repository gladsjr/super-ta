...
import { expectedSchema, readMigrationFiles, diffExpected, readCatalog } from "./schemaExpectations.js";
...
export const EXPECTED_SCHEMA = expectedSchema(readMigrationFiles(path.join(PROJECT_ROOT, "migrations")));
...
async function checkSchema(ctx) {
    ...
    // coluna que não existe. Confere no CATÁLOGO que tudo o que as migrations
...
async function checkSeeds(ctx) {
    ...
    const status = (ausentes.length || vazias.length || adminBootstrap !== true) ? "fail" : "ok";
...
//
// Prazo folgado e diagnóstico completo: em produção (07/09) a sonda de 2 s
// estourou em TODAS as chamadas com ffmpeg-full instalado e a fila de vídeo
// funcionando — "não achei" e "achei mas demorou" são coisas diferentes, e
// a sonda precisa dizer qual foi (código do erro, sinal, tempo).
const FFMPEG_PROBE_MS = 8000;
let ffmpegVisto = null;
export function _resetFfmpegProbe() { ffmpegVisto = null; } // só para teste
function versaoFfmpeg() {
    if (ffmpegVisto) return Promise.resolve({ version: ffmpegVisto.version, ms: ffmpegVisto.ms, cached: true });
    const t0 = Date.now();
    return new Promise((resolve) => {
        try {
            execFile("ffmpeg", ["-version"], { encoding: "utf8", timeout: FFMPEG_PROBE_MS }, (err, stdout) => {
                const ms = Date.now() - t0;
                if (err) return resolve({ version: null, ms, error: err.code || err.message, signal: err.signal || null, timed_out: !!err.killed });
                const version = (String(stdout).split("\n")[0] || "").replace(/^ffmpeg version\s+/, "").split(" ")[0] || null;
                if (version) ffmpegVisto = { version, ms };
                resolve({ version, ms });
            });
        } catch (err) { resolve({ version: null, ms: Date.now() - t0, error: err.message }); }
    });
}
...
async function checkAssets() {
    ...
    const sonda = await versaoFfmpeg();
    const faltando = arquivos.filter(a => !a.ok).map(a => a.path);
    const status = (faltando.length || mp3 < 10 || !sonda.version) ? "fail" : "ok";
    return resultado(status, { missing: faltando, soundcheck_mp3: mp3, soundcheck_expected: 10, ffmpeg: sonda.version, ffmpeg_probe: sonda });
}
...
// `budget_ms` é o prazo externo do check quando o padrão (SHALLOW_TIMEOUT_MS)
// não cabe: o de assets tem de ser MAIOR que a sonda do ffmpeg, senão o
// diagnóstico da sonda nunca chega ao relatório (revisão do #390).
export const CHECKS = [
    ...
    { id: "assets",     label: "Binários e modelos",            level: "shallow", db: false, run: checkAssets, budget_ms: FFMPEG_PROBE_MS + 1000 },
