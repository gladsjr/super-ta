...
export async function runHealth({ ids = null, depth = "shallow" } = {}) {
    ...
    const escolhidos = CHECKS.filter(c => niveis.includes(c.level) && (!ids || ids.includes(c.id)));
    const t0 = Date.now();
    ...
    try {
        ...
        const semBanco = (async () => {
            if (depth !== "shallow") {
                ...
                // Orçamento mensal do trabalho de saúde, UMA leitura antes de
                // qualquer check pago (#375, corte 4). Sem banco, fica null e os
                // checks pagos rodam sem contabilizar — o check `budget` acusa.
            ...
            return Promise.all(escolhidos.filter(c => !c.db).map(c => executarUm(c, ctx)));
        })();
