...
export async function checkRealtimeB(ctx = {}) {
    ...
    const t0 = Date.now();
    const apagadas = await ops.limparSondasAntigas(workId);
    const sub = await ops.criarSonda(workId);
    ...
    let custo = { cost_usd: 0, events: 0 }, estimado = false;
    try {
        custo = await ops.custoDaSonda(sub.id);
        if (custo.events === 0 && audioBytes > 0) {
            // A fala foi cortada antes do response.done: o relay não pôde medir.
            await ops.registrarEstimativa({ workId, submissionId: sub.id, audioSeconds: audioBytes / 48000 });
            custo = await ops.custoDaSonda(sub.id);
            estimado = true;
        }
    } catch (err) { log.warn("HEALTH", `custo da sonda não lido/estimado: ${err.message}`); }
    ...
    const detail = {
        ...
        realtime_cost_events: custo.events, cost_estimated: estimado, billed_to_work: workId,
    };
