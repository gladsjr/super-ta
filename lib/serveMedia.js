// Entrega de MÍDIA com HTTP Range de verdade (#349, #376, #380).
//
// Serve vídeo de fiscalização E áudio de resposta do aluno. Nasceu só para
// vídeo — daí o nome antigo, `serveVideo` —, e o áudio ficou de fora servindo
// arquivo cru: sem Content-Length, sem Accept-Ranges, ignorando Range. O
// Safari, que manda `Range: bytes=0-1` ao abrir um elemento de mídia e espera
// 206, simplesmente errava; o Chrome tocava sem duração (00:00). Renomeado para
// que ninguém mais leia "vídeo" e conclua que áudio tem outro caminho.
//
...
// `tamanhoConhecido` evita ir ao storage quando quem chama já sabe o tamanho —
// é o caso do áudio, cujo `byte_size` está gravado em student_audio_artifacts
// desde o arquivamento. Mais barato e mais confiável que medir.
export async function serveMedia(req, res, key, contexto = "", { tamanhoConhecido = null, mimetype = null } = {}) {
    // O tipo vem de QUEM CHAMA quando ele o conhece. Derivar da extensão basta
    // para vídeo, mas áudio e vídeo compartilham o `.webm`: sem isto, a resposta
    // de um áudio sai como `video/webm` e o elemento <audio> recebe um tipo que
    // não é o dele. `student_audio_artifacts.mimetype` guarda o valor correto
    // desde o arquivamento.
    const type = mimetype || videoMimeFromKey(key);
    let tamanho = Number.isFinite(tamanhoConhecido) && tamanhoConhecido > 0 ? tamanhoConhecido : await getObjectSize(key);
