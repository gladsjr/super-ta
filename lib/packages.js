...
// Cada trava aqui TEM de ter um consumidor no runtime (critério do #152). Chave
// fora desta lista é falha na carga: um pacote não pode prometer uma restrição
// que o motor ignora silenciosamente (afeta custo, comparação de ofertas e
// consistência comercial). Ao adicionar uma trava nova, ligue o consumidor no
// mesmo passo.
const KNOWN_LOCK_KEYS = new Set([
    "question_count",    // → works.question_count → teto de turnos (routes/interview.js)
    "interaction_mode",  // → works.interaction_mode → texto|áudio
    "max_follow_ups",    // → works.max_follow_ups → teto total de follow-ups/pergunta (routes/interview.js)
    "evaluation_mode",   // → works.evaluation_mode → requireEvaluationOutputsAllowed
    "grade",             // → nota (prova oral)
]);
...
function validateLocks(itemKey, locks) {
    ...
    for (const k of Object.keys(locks)) {
        if (!KNOWN_LOCK_KEYS.has(k)) {
            fail(`item "${itemKey}" trava desconhecida "${k}" — toda trava precisa de um consumidor no runtime (conhecidas: ${[...KNOWN_LOCK_KEYS].join(", ")})`);
        }
    }
    const out = {};
