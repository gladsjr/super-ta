    ...
    const upd = await client.query(
        `UPDATE entitlement_counters ec
          ...
          WHERE ec.id = $1 AND pa.id = ec.allocation_id
            AND (ec.consumed_qty + $2) <= (pa.granted_packages - pa.delegated_packages) * ec.item_quantity
         RETURNING ec.id`,
        [counter.id, n]
    );
    if (upd.rowCount === 0) {
        const av = await client.query(
            `SELECT (pa.granted_packages - pa.delegated_packages) * ec.item_quantity - ec.consumed_qty AS available
               FROM entitlement_counters ec JOIN package_allocations pa ON pa.id = ec.allocation_id
              WHERE ec.id = $1`,
            [counter.id]
        );
        return { ok: false, reason: "exhausted", itemKey: w.ik, needed: n, available: Math.max(0, Number(av.rows[0]?.available ?? 0)) };
    }
    for (const sid of submissionIds) {
        await client.query(
            `INSERT INTO entitlement_consumption (counter_id, item_key, work_id, submission_id, drawn_by_user_id, locks_json)
             VALUES ($1,$2,$3,$4,$5,$6)`,
            [counter.id, w.ik, workId, sid, byUserId, JSON.stringify(counter.locks_json || {})]
        );
    }
...
export async function releaseForSubmission(submissionId) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
...
export async function releaseForWork(workId) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const rows = await client.query(
