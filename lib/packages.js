...
// ---------------------------------------------------------------------------
// Consumo (saque) — chamado no início da execução (Gate B)
// ---------------------------------------------------------------------------

// Saca 1 unidade do item `itemKey` do pool da unidade `unitId` sob o template
// `templateKey`. Atômico e conservador: só saca se houver disponível. Retorna
// { ok:true, locks } em sucesso, ou { ok:false, reason } se esgotado/inexistente.
export async function drawEntitlement({ unitId, templateKey, itemKey, workId = null, submissionId = null, drawnByUserId = null }) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const found = await client.query(
            `SELECT ec.id, ec.item_quantity, ec.locks_json
              ...
              LIMIT 1
              FOR UPDATE OF ec`,
            [unitId, templateKey, itemKey]
        );
        if (found.rowCount === 0) {
            await client.query("ROLLBACK");
            return { ok: false, reason: "no_counter" };
        }
        const counterId = found.rows[0].id;
        const locks = found.rows[0].locks_json || {};
        const upd = await client.query(
            `UPDATE entitlement_counters ec
                SET consumed_qty = consumed_qty + 1
               FROM package_allocations pa
              WHERE ec.id = $1 AND pa.id = ec.allocation_id
                AND (ec.consumed_qty + 1) <= (pa.granted_packages - pa.delegated_packages) * ec.item_quantity
             RETURNING ec.id`,
            [counterId]
        );
        if (upd.rowCount === 0) {
        ...
        await client.query(
            `INSERT INTO entitlement_consumption (counter_id, item_key, work_id, submission_id, drawn_by_user_id, locks_json)
             VALUES ($1,$2,$3,$4,$5,$6)`,
            [counterId, itemKey, workId, submissionId, drawnByUserId, JSON.stringify(locks)]
        );
        await client.query("COMMIT");
...
// Wrapper de saque para um TRABALHO: lê o binding de pacote do work e saca. Se o
// trabalho não está sob pacote (sem template/item/unit) → { ok:true, skipped:true }
// (não há Portão B, só o orçamento US$). Não saca duas vezes para a mesma
// submissão (guard de idempotência — /upload pode repetir numa tentativa).
export async function drawForWork({ workId, submissionId = null, drawnByUserId = null }) {
    ...
    if (submissionId != null) {
        const ex = await pool.query(
            `SELECT 1 FROM entitlement_consumption WHERE submission_id = $1 LIMIT 1`,
            [submissionId]
        );
        if (ex.rowCount > 0) return { ok: true, skipped: true, already: true };
    }
    return drawEntitlement({
        unitId: w.unit_id, templateKey: w.tk, itemKey: w.ik,
        workId, submissionId, drawnByUserId,
    });
}
...
export async function listUnitEntitlements(unitId) {
    const allocs = await pool.query(
        `SELECT pa.id, pa.template_key, pa.template_version,
                pa.granted_packages, pa.delegated_packages,
