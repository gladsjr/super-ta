...
import fs from "fs";
import { assertProviderReady } from "./sttProvider.js";
...
// Camada de provedor STT (#284). Defaults preservam o comportamento histórico
// (openai, sem fallback, sem sombra). Provedor configurado sem credencial ou
// preço derruba o boot — validação em assertProviderReady + pricing abaixo.
const _sttProv = policy?.models?.stt_provider ?? "openai";
const _sttFall = policy?.models?.stt_fallback_provider ?? null;
const _sttShadow = policy?.models?.stt_shadow_provider ?? null;
for (const [key, v] of [["stt_provider", _sttProv], ["stt_fallback_provider", _sttFall], ["stt_shadow_provider", _sttShadow]]) {
    if (v != null && !["openai", "groq"].includes(v)) {
        throw new Error(`config/policy.yaml models.${key} inválido: "${v}" (use openai|groq ou remova)`);
    }
}
export const STT_PROVIDER = _sttProv;
export const STT_FALLBACK_PROVIDER = _sttFall;
export const STT_SHADOW_PROVIDER = _sttShadow;
export const STT_GROQ_MODEL = policy?.models?.stt_groq_model ?? "whisper-large-v3";
export const STT_TIMEOUT_MS = Math.max(0, Math.floor(Number(policy?.models?.stt_timeout_ms ?? 0)));
const _shadowRate = Number(policy?.models?.stt_shadow_rate ?? 0);
if (Number.isNaN(_shadowRate) || _shadowRate < 0 || _shadowRate > 1) {
    throw new Error(`config/policy.yaml models.stt_shadow_rate inválido: "${policy?.models?.stt_shadow_rate}" (use 0..1)`);
}
export const STT_SHADOW_RATE = _shadowRate;
// Fila de jobs (#289, corte 3). O motor 'local' NÃO é validado no boot (o
// python/faster-whisper pode nem existir na VM): o executor tenta e degrada
// para 'api' com log — é batch de auditoria, não caminho crítico (a exceção
// deliberada à ADR 0002, que protege caminhos de sessão).
const _retxEngine = policy?.jobs?.retranscribe_engine ?? "api";
if (!["api", "local"].includes(_retxEngine)) {
    throw new Error(`config/policy.yaml jobs.retranscribe_engine inválido: "${_retxEngine}" (use api|local)`);
}
export const RETRANSCRIBE_ENGINE = _retxEngine;

// Teto mensal do trabalho de saúde (#375): soma do ledger no mês corrente.
export const HEALTH_MONTHLY_BUDGET_USD = Math.max(0, Number(policy?.health?.monthly_budget_usd ?? 1.0));
export const JOBS_LOCAL_MIN_FREE_MB = Math.max(0, Math.floor(Number(policy?.jobs?.local_min_free_mb ?? 700)));
export const JOBS_POLL_SECONDS = Math.max(10, Math.floor(Number(policy?.jobs?.poll_seconds ?? 60)));
...
// ttl "30m" sozinho no payload do orquestrador (sem breakpoints). Hipótese Replit.
export const PROMPT_CACHE_TTL30M = policy?.prompt_cache?.ttl_30m === true;
...
log.info("CONFIG", `jobs retranscribe_engine=${RETRANSCRIBE_ENGINE} poll=${JOBS_POLL_SECONDS}s local_min_free_mb=${JOBS_LOCAL_MIN_FREE_MB}`);
...
loadPricing();
// #284: todo provedor de STT CONFIGURADO (primário, fallback ou sombra) entra
// na cobertura de preço e tem a credencial validada AGORA — não na primeira
// chamada (ADR 0002; lição do metering realtime zerado).
const _sttModels = new Set([STT_MODEL]);
for (const prov of new Set([STT_PROVIDER, STT_FALLBACK_PROVIDER, STT_SHADOW_PROVIDER].filter(Boolean))) {
    const m = assertProviderReady(prov, { groqModel: STT_GROQ_MODEL });
    if (m) _sttModels.add(m);
}
validatePricingCoverage({
    textModels: [PRINCIPAL_REASONING_MODEL, FAST_MODEL],
    sttModels: [..._sttModels],
    ttsModels: [TTS_MODEL],
    realtimeModels: [REALTIME_MODEL],
});
...
// A CHECK constraint em migrations/049 reforça os mesmos limites no banco
// (works_question_count_3_10_chk; a 010 original permitia 3..20 — o cap caiu
// para 10 no PR #116 e a 049 alinhou o schema).
export const DEFAULT_QUESTION_COUNT = 6;
export const MIN_QUESTION_COUNT = 3;
export const MAX_QUESTION_COUNT = 10;
// Geração de rubricas EM BLOCO (issue #194): quantas questões por chamada ao
// modelo. 5 é ponto de partida (corta o overhead de raciocínio ~5× sem diluir
// demais o raciocínio por questão); configurável para calibrar por dado depois.
export const ORAL_RUBRIC_BLOCK_SIZE = Math.max(1, Math.floor(Number(process.env.ORAL_RUBRIC_BLOCK_SIZE) || 5));
