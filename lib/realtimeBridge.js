...
import { createAudioTee } from "./audioTee.js";
import { erroFatalDoProvedor } from "./providerErrors.js";
...
// Sessões de voz ATIVAS no processo inteiro (soma dos relays oral + entrevista).
// A janela ociosa do executor de jobs (#289 corte 3) consulta isto: batch
// pesado só roda com ZERO sessões de voz — a regra do ADR 0021 aplicada à CPU.
// (Os conjuntos por relay já existem em ACTIVE_BY_SCOPE — somamos deles.)
export function getActiveVoiceSessions() {
    let n = 0;
    for (const set of ACTIVE_BY_SCOPE.values()) n += set.size;
    return n;
}
...
// Marca a resposta cuja transcrição falhou (#359). Precisa ser legível pelo
// avaliador e pelo professor: silêncio no lugar dela seria lido como "o aluno
// não respondeu", que é exatamente o erro que queremos evitar.
const FALA_NAO_TRANSCRITA = "[falha na transcrição desta resposta — o aluno falou, mas o áudio não foi convertido em texto]";

// Monta a config da sessão Realtime (server-to-server). Sem legenda ao aluno.
// Exportada para a perna A do health check (#375): o session.update da sonda
// tem de ser o MESMO que o relay manda, senão a sonda passa e a prova falha.
export function buildSessionConfig({ instructions, voice, endTool }) {
    return {
        type: "realtime",
...
export function bridge(clientWs, cfg) {
    ...
    const CONFIRM_MS = 8000;   // espera pelo session.updated antes de seguir sem ele (#351)
    ...
    // Confirmação da configuração da sessão (#351). Enquanto não confirmada, um
    // "error" da OpenAI é FATAL: significa que o session.update foi recusado e o
    // modelo está com a configuração padrão dele, não com a nossa.
    let sessionConfirmada = false;
    let sessionRejeitada = false;
    let primeiraFalaPedida = false;
    ...
    let studentSpeechEvents = 0; // eventos de fala do aluno (VAD), independentes de texto (#283)
    // Falhas de transcrição da fala do aluno (#359). Vão aos sinais de voz: o
    // professor precisa saber, ANTES de publicar a nota, que parte do que o
    // aluno disse não chegou ao avaliador.
    let sttFailures = 0;
    // TEE de áudio do aluno (#289): cópia da corrente PCM + índice de
    // fronteiras, fora do caminho crítico (falha = log; o relay nunca espera).
    // Só é criado quando o adapter quer persistir o resultado (onTeeAudio).
    const tee = cfg.onTeeAudio ? createAudioTee({ token, scope: SCOPE }) : null;
    ...
    // Pede a apresentação + 1ª pergunta. O modelo não inicia sozinho. Idempotente:
    // a confirmação e o timeout podem correr os dois.
    const iniciarPrimeiraFala = (motivo) => {
        if (primeiraFalaPedida || sessionRejeitada) return;
        primeiraFalaPedida = true;
        clearTimeout(confirmTimer);
        try { oai.send(JSON.stringify({ type: "response.create" })); } catch {}
        log.info(SCOPE, `1ª fala pedida submission=${token} (${motivo})`);
    };

    // Configuração recusada: a sessão NÃO pode continuar. Um agente genérico
    // conduzindo a arguição em inglês é pior do que uma sessão que não abre —
    // a segunda o aluno percebe e reporta; a primeira passa por prova válida.
    // Encerra por falha do PROVEDOR já com a sessão em andamento (#359). Difere
    // do abort de configuração só no motivo que chega ao cliente: aqui a prova
    // COMEÇOU, então a tela precisa dizer que o corte foi técnico — e não que a
    // arguição terminou.
    const abortarPorProvedor = (detalhe) => {
        if (sessionRejeitada) return;
        sessionRejeitada = true;
        log.error(SCOPE, `provedor falhou COM A SESSÃO EM ANDAMENTO submission=${token}: ${detalhe}`);
        sendClient({ type: "ended", reason: "provider_error" });
        try { oai.close(); } catch {}
        try { clientWs.close(); } catch {}
    };

    const abortarPorConfigRecusada = (detalhe) => {
        if (sessionRejeitada) return;
        sessionRejeitada = true;
        clearTimeout(confirmTimer);
        log.error(SCOPE, `configuração da sessão RECUSADA submission=${token}: ${detalhe}`);
        sendClient({ type: "ended", reason: "config_rejeitada" });
        try { oai.close(); } catch {}
        try { clientWs.close(); } catch {}
    };
    ...
    oai.on("open", () => {
        oaiOpen = true;
        // A 1ª resposta só é pedida DEPOIS que a OpenAI confirmar a configuração
        // (#351). Antes, `session.update` e `response.create` saíam juntos: quando
        // a configuração era rejeitada — bastava uma voz que a TTS aceita e o
        // Realtime não —, a rejeição virava uma linha de log e a arguição seguia
        // com a personalidade PADRÃO do provedor, em inglês, sem as questões e sem
        // a ferramenta de encerramento. Ninguém era avisado.
        ...
        log.info(SCOPE, `bridge open submission=${token} n=${n} voice=${voice} (aguardando session.updated)`);
        // Rede de segurança: se a confirmação não vier (protocolo mudou, evento
        // perdido), seguimos assim mesmo em vez de travar a sessão — mas o log
        // registra, porque aí a configuração não está garantida.
        confirmTimer = setTimeout(() => {
            if (sessionConfirmada || sessionRejeitada) return;
            log.warn(SCOPE, `session.updated não chegou em ${CONFIRM_MS}ms submission=${token} — seguindo sem confirmação`);
            iniciarPrimeiraFala("timeout");
        }, CONFIRM_MS);
    ...
    oai.on("message", (data, isBinary) => {
        ...
        switch (m.type) {
            ...
            case "input_audio_buffer.speech_started":
                ...
                // Contador de EVENTOS de fala do aluno (#283): independe da
                // transcrição virar texto — é o que o gate de retomada (#260)
                // consulta em vez do transcript, para uma sessão 100% alucinada
                // (fala real, texto lixo) ainda contar como "aluno falou".
                studentSpeechEvents++;
            ...
            case "conversation.item.input_audio_transcription.completed":
                ...
                break;
            // A transcrição da fala do aluno FALHOU (#359).
            //
            // Sem tratar, o slot reservado ficava com texto vazio e era filtrado
            // no fechamento — a resposta sumia sem deixar rastro, a prova seguia
            // e era avaliada COMO SE o aluno não tivesse respondido. Uma prova
            // que não abre o aluno reclama; uma avaliada sem as respostas dele
            // passa por legítima e vira nota.
            //
            // Duas marcas, porque servem a leitores diferentes: o marcador no
            // transcript diz ao avaliador que houve fala não capturada (ausência
            // de texto ≠ ausência de resposta), e o contador vai aos sinais de
            // voz, onde o professor o vê antes de publicar.
            case "conversation.item.input_audio_transcription.failed": {
                sttFailures++;
                const detalhe = JSON.stringify(m.error || {}).slice(0, 200);
                log.error(SCOPE, `transcrição da fala do aluno falhou submission=${token} n=${sttFailures}: ${detalhe}`);
                const slot = m.item_id ? transcript.find(t => t._item === m.item_id) : null;
                if (slot) { slot.text = FALA_NAO_TRANSCRITA; delete slot._item; }
                else transcript.push({ role: "student", text: FALA_NAO_TRANSCRITA });
                // Conta como turno do aluno: ele FALOU. Não contar aqui faria o
                // gate de retomada (#283) tratar a sessão como se nada tivesse
                // acontecido.
                studentTurns++; silenceNudges = 0;
                break;
            }
            ...
            case "response.done": {
                ...
                tee?.mark("examiner_done"); // #289: fronteira de pergunta no índice
            ...
            // Confirmação de que a NOSSA configuração entrou (#351). Só depois
            // dela o modelo é mandado falar.
            case "session.updated":
                if (!sessionConfirmada) {
                    sessionConfirmada = true;
                    clearTimeout(confirmTimer);
                    iniciarPrimeiraFala("session.updated");
                }
                break;
            case "error": {
                const detalhe = JSON.stringify(m.error || m).slice(0, 300);
                // Antes da confirmação, erro = session.update recusado (#351).
                if (!sessionConfirmada) return abortarPorConfigRecusada(detalhe);
                // DEPOIS da confirmação o #351 parava aqui, e era só meia
                // correção (#359): saldo que acaba no MEIO da prova voltava a
                // ser "log e segue". A sessão continuava viva e MUDA — o
                // keep-alive segurando uma conexão que nunca mais responde —
                // enquanto o aluno falava para o nada, sem nada na tela.
                //
                // Erro fatal do provedor não tem recuperação deste lado: sai
                // agora, dizendo o que houve. O aluno reentra quando o
                // professor resolver, e a gravação já feita está preservada.
                if (erroFatalDoProvedor(m.error)) return abortarPorProvedor(detalhe);
                log.error(SCOPE, `openai error submission=${token}: ${detalhe}`);
                break;
            }
        }
    });

    // Fechamento do lado do PROVEDOR. O código/motivo vai para o log (#343): sem
    // ele, uma sessão que morre cedo é indistinguível de queda do navegador — o
    // diagnóstico dependia do relato do aluno. O `reason` também viaja ao
    // cliente, que avisa que foi problema técnico (e não fim da entrevista).
    oai.on("close", (code, reasonBuf) => {
        clearInterval(ka); clearInterval(watchdog); clearTimeout(confirmTimer);
        const reason = String(reasonBuf || "").slice(0, 120);
        const expected = endRequested || cleanFinish;
        log[expected ? "info" : "warn"](SCOPE, `openai ws fechou code=${code || "—"}${reason ? ` reason="${reason}"` : ""} esperado=${expected} submission=${token}`);
        sendClient({ type: "ended", reason: expected ? "normal" : "upstream_closed" });
        try { clientWs.close(); } catch {}
    });
    ...
    clientWs.on("message", (data, isBinary) => {
        ...
        if (isBinary) {
            ...
            tee?.write(data); // #289: cópia p/ retranscrição (nunca bloqueia)
        } else {
    ...
    clientWs.on("close", async (code, reasonBuf) => {
        clearInterval(ka); clearInterval(watchdog); clearTimeout(confirmTimer);
        // Código/motivo no log (#343): 1000/1001 = saída normal do navegador;
        // 1006 = queda abrupta (rede). Sem isto não dava p/ distinguir queda do
        // aluno de fechamento vindo do provedor.
        {
            const reason = String(reasonBuf || "").slice(0, 120);
            log[cleanFinish ? "info" : "warn"](SCOPE, `navegador ws fechou code=${code || "—"}${reason ? ` reason="${reason}"` : ""} limpo=${cleanFinish} submission=${token}`);
        }
        if (endingTimer) { clearTimeout(endingTimer); endingTimer = null; }
        try { oai.close(); } catch {}
        ...
        // A persistência da transcrição GOVERNA a conclusão (#359): sem ela a
        // arguição existiu para o aluno e não existe para o sistema.
        let transcriptPersistido = true;
        if (finalTranscript.length > 0) {
            try { await cfg.onTranscript(finalTranscript, { cleanFinish }); }
            catch (err) {
                transcriptPersistido = false;
                log.error(SCOPE, `onTranscript failed submission=${token}: ${err.message}`);
            }
        }
        ...
        try {
            ...
            // #283: o gate de retomada consulta este contador em vez do texto.
            signals.student_speech_events = studentSpeechEvents;
            // #359: respostas que o aluno deu e que NÃO viraram texto. Vai aos
            // sinais para o professor ver antes de publicar a nota — avaliação
            // sem insumo completo não pode passar por completa em silêncio.
            if (sttFailures > 0) signals.transcription_failures = sttFailures;
            if (nudgesState) {
        ...
        // TEE (#289): converte e sobe o áudio contínuo do aluno — pós-sessão,
        // o relay já fechou o lado OpenAI; falha é log, nunca derruba o resto.
        if (tee) {
            try {
                const teeResult = await tee.finish();
                if (teeResult) await cfg.onTeeAudio(teeResult, { cleanFinish });
            } catch (err) { log.error(SCOPE, `onTeeAudio failed submission=${token}: ${err.message}`); }
        }
        ...
        if (cleanFinish && usageAcc.length > 0) {
            // Não concluir sem a transcrição salva (#359). Antes, uma falha ao
            // persistir virava só uma linha de log e a submissão era marcada
            // como concluída assim mesmo — depois a avaliação rodava sobre
            // nada e produzia nota. Deixando pendente, o aluno não perde a
            // tentativa e o professor vê que algo ficou por resolver.
            if (!transcriptPersistido) {
                log.error(SCOPE, `NÃO concluída: a transcrição não foi salva submission=${token}`);
            } else {
...
// Registro GLOBAL de sessões de voz vivas, por escopo (ORAL_RELAY/LIVE_RELAY) —
// alimenta o dashboard de Operações (issue #265). O dono do estado continua
// sendo o Set de cada relay; aqui só se expõe a contagem.
const ACTIVE_BY_SCOPE = new Map(); // scope -> Set de tokens
export function getActiveRealtimeSessions() {
    const out = {};
    for (const [scope, set] of ACTIVE_BY_SCOPE) out[scope] = set.size;
    return out;
}
...
export function attachRealtimeRelay(httpServer, adapter) {
    ...
    ACTIVE_BY_SCOPE.set(adapter.scope, activeSessions);
