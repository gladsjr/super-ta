...
export function bridge(clientWs, cfg) {
    ...
    let paused = false;       // CAPTURA de vídeo perdida (proctoring obrigatório): a
                              // arguição fica em espera — não encaminha o áudio do
                              // aluno nem cutuca — até a câmera religar (capture_ok).
    ...
    clientWs.on("message", (data, isBinary) => {
        ...
        if (isBinary) {
            ...
            else if (m.type === "capture_lost") { if (!paused) { paused = true; log.info(SCOPE, `captura de vídeo perdida — arguição pausada submission=${token}`); } }
            else if (m.type === "capture_ok") { if (paused) { paused = false; lastVoiceAt = Date.now(); examinerDoneAt = Date.now(); log.info(SCOPE, `captura de vídeo retomada submission=${token}`); } }
        }
    });
    ...
    clientWs.on("close", async () => {
        ...
        try {
            await cfg.onVoiceSignals(lat.summary());
