...
export function createAudioTee({ token, scope = "TEE" }) {
    ...
    return {
        ...
        // Além do relógio de parede, grava a POSIÇÃO NO ÁUDIO em bytes (b):
        // b/48000 = segundo exato no arquivo — o relógio de parede deriva da
        // posição de áudio quando a captura pausa; o fatiamento (#289 corte 2)
        // usa b, nunca t_ms.
        mark(type) {
            if (dead) return;
            index.push({ t_ms: Date.now() - startedAt, b: bytes, type });
        },
...
function pcmToOgg(pcmPath) {
    return new Promise((resolve, reject) => {
        ...
        // spawnLow (#289 corte 3): nice 19 + 1 thread — o encode nunca disputa
        // CPU com uma sessão ao vivo que porventura exista.
