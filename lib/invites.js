...
export async function createPersonWithInvite({ name, email, role, unitId, createdByUserId = null }) {
    ...
    // Tudo-ou-nada (issue #159): find/create user + vínculo + convite numa ÚNICA
    // transação. Antes eram queries independentes no pool — uma falha no meio
    // deixava conta órfã, vínculo sem convite, ou o convite anterior cancelado
    // sem substituto. Uma corrida de username agora aborta a transação limpa
    // (ROLLBACK) em vez de deixar estado parcial.
    ...
    try {
        ...
        let user = (await client.query(`SELECT * FROM users WHERE lower(email) = lower($1) LIMIT 1`, [em])).rows[0] || null;
        if (!user) {
        ...
        const membership = await addMembership({ userId: user.id, unitId, role }, client);

        // Convite só se a conta ainda não tem porta de entrada.
        let invite = null;
        const hasIdentity = (await client.query(`SELECT 1 FROM user_identities WHERE user_id = $1 LIMIT 1`, [user.id])).rowCount > 0;
        if (!user.password_hash && !hasIdentity) {
            invite = await issueInvite({ userId: user.id, email: em, createdByUserId }, client);
        }
...
// client opcional (default pool): participa de uma transação externa quando
// chamado dentro de createPersonWithInvite (issue #159).
export async function issueInvite({ userId, email = null, createdByUserId = null }, client = pool) {
    const em = email || (await client.query(`SELECT email FROM users WHERE id = $1`, [userId])).rows[0]?.email;
    ...
    const r = await client.query(
        `INSERT INTO invites (user_id, email, token, created_by_user_id, expires_at)
         VALUES ($1, $2, $3, $4, now() + interval '${INVITE_TTL_DAYS} days')
         RETURNING *`,
        [userId, em, token, createdByUserId]
    );
    return { ...r.rows[0], state: "pendente" };
}
...
export async function listUnitInvites(unitId) {
    const r = await pool.query(
        `SELECT DISTINCT ON (i.user_id) i.id, i.user_id, i.email, i.token,
