...
export async function createPersonWithInvite({ name, email, role, unitId, civilIdValue = null, createdByUserId = null }) {
    ...
    // Pessoa única por e-mail: reaproveita se já existe (não duplica cadastro).
    ...
    if (!user) {
        ...
        user = (await pool.query(
            `INSERT INTO users (username, email, display_name, civil_id_type, civil_id_value, source)
             VALUES ($1, $2, $3, $4, $5, 'manual') RETURNING *`,
            [candidate, em, nm, civilIdValue ? "cpf" : null, civilIdValue || null]
        )).rows[0];
    }
...
// Ativa a conta: define a senha e consome o convite. Atômico.
export async function activateInvite(token, password, bcrypt) {
