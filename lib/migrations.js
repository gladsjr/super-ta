...
// Irmão SOMENTE-LEITURA de listMigrationStatus, para o health check (#375).
//
// A versão de CLI não serve ao servidor por dois motivos: tem o guard
// assertCliContext (MIGRATIONS_CLI=1) e roda CREATE TABLE IF NOT EXISTS antes
// de ler — e o boot NÃO pode executar DDL (ADR 0001). Aqui só se lê: lista os
// arquivos, lê schema_migrations, compara. Se a tabela não existir, isso é um
// RESULTADO (o banco nunca foi migrado), não uma exceção a engolir.
export async function listMigrationStatusReadOnly(q = (sql) => pool.query(sql)) {
    const all = listMigrationFiles();
    let appliedSet;
    try {
        const r = await q("SELECT version FROM schema_migrations");
        appliedSet = new Set(r.rows.map((row) => row.version));
    } catch (err) {
        if (/relation .*schema_migrations.* does not exist/i.test(err.message)) {
            return { tableMissing: true, migrations: all.map((m) => ({ version: m.version, filename: m.filename, applied: false })) };
        }
        throw err;
    }
    return {
        tableMissing: false,
        migrations: all.map((m) => ({ version: m.version, filename: m.filename, applied: appliedSet.has(m.version) })),
    };
}
