...
export async function createWork(name, budgetUsd, kind = "interview", opts = {}) {
    ...
    const r = await pool.query(
        `INSERT INTO works (work_token, name, budget_usd, kind, is_benchmark, benchmark_run_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, work_token, name, kind, budget_usd::float8 AS budget_usd, spent_usd::float8 AS spent_usd, is_benchmark, benchmark_run_id`,
        [token, name, budgetUsd, kind, isBenchmark, benchmarkRunId]
    );
    return r.rows[0];
}
