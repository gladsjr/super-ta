...
import { normalizeRubricLevels, questionHasRubric, sameRubric } from "../oralRubric.js";
...
// Trabalho de saúde (is_health) não é desativável nem aqui: a rota devolve 409
// antes, e o SQL garante (#375).
export async function setWorkActive(workId, isActive) {
    const r = await pool.query(
        `UPDATE works
         SET is_active = $1, updated_at = now()
         WHERE id = $2 AND NOT is_health
         RETURNING is_active`,
        [!!isActive, workId]
    );
...
export async function deleteWork(workId, client = pool) {
    const r = await client.query("DELETE FROM works WHERE id = $1 AND NOT is_health", [workId]);
    return r.rowCount > 0;
}
...
export async function setOralQuestions(workId, questions) {
    ...
    const cleaned = kept.map(q => {
        ...
        const rubric = String(q?.rubric ?? "").trim();        // string legada (compat/fallback)
        const rubric_levels = normalizeRubricLevels(q?.rubric_levels); // 5 níveis (#195)
        const cur = { rubric, rubric_levels };
        const prev = id != null ? prevById[id] : null;
        let rubric_stale;
        if (!prev) rubric_stale = questionHasRubric(cur) ? false : true;
        else if (!sameRubric(cur, prev)) rubric_stale = false;   // rubrica mudou → o professor definiu/revisou
        ...
        return { id, question, answer, rubric, rubric_levels, weight: Number(q?.weight) > 0 ? Number(q.weight) : 1, rubric_stale };
    });
    await pool.query(
...
// os 5 níveis (#195) no item de id `qid`, limpa a string legada e o rubric_stale.
// Não mexe nos demais.
export async function updateOralQuestionRubric(workId, qid, { rubric_levels, weight }) {
    ...
    const next = arr.map(q => {
        ...
        touched = true;
        return { ...q, rubric: "", rubric_levels: normalizeRubricLevels(rubric_levels), weight: Number(weight) > 0 ? Number(weight) : (Number(q.weight) > 0 ? Number(q.weight) : 1), rubric_stale: false };
    });
