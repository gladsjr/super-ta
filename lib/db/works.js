...
// Perguntas da prova oral (JSONB [{id,question,answer,weight}]). As respostas/critérios
// ficam aqui; nunca vão à sessão Realtime do aluno.
//
...
// (grades_json / oral_asked_json) continuam apontando a mesma questão. Devolve o
// array já com ids (o chamador usa na resposta).
export async function setOralQuestions(workId, questions) {
    const incoming = Array.isArray(questions) ? questions : [];
    // Descarta questões sem enunciado ANTES de atribuir ids (não consome ids à toa).
    const kept = incoming.filter(q => String(q?.question ?? "").trim());
    const r = await pool.query("SELECT oral_next_qid FROM works WHERE id = $1", [workId]);
