...
// spec_json do template (entitlement_counters.locks_json). Idempotente por UPDATE.
export async function applyWorkPackageBinding(workId, templateKey, itemKey, locks = {}) {
    ...
    await pool.query(
        `UPDATE works SET
            ...
            updated_at = now()
          WHERE id = $7`,
        [templateKey, itemKey, qc, mode, mfu, evalMode, workId]
    );
}
