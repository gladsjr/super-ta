...
// Tamanho de um objeto do storage, por chave (#349). Gravado no momento em que
// o objeto é criado, quando o buffer ainda está em mãos — é o que permite
// responder Content-Range sem baixar nada para medir.
export async function setObjectSize(key, bytes) {
    if (!key || !Number.isFinite(bytes) || bytes < 0) return;
    await pool.query(
        `INSERT INTO object_sizes (object_key, bytes) VALUES ($1, $2)
         ON CONFLICT (object_key) DO UPDATE SET bytes = EXCLUDED.bytes`,
        [key, Math.round(bytes)]
    );
}

// null quando o objeto é anterior ao #349 — o caller então serve inteiro.
export async function getObjectSize(key) {
    if (!key) return null;
    try {
        const r = await pool.query("SELECT bytes FROM object_sizes WHERE object_key = $1", [key]);
        const n = r.rows[0]?.bytes;
        return n == null ? null : Number(n);
    } catch (err) {
        // A migration 080 é um passo separado do deploy: entre o código subir e ela
        // rodar, esta tabela não existe. Sem esta guarda, TODA requisição de vídeo
        // estoura nessa janela. Tamanho desconhecido só custa o seek (a rota serve
        // o objeto inteiro em stream) — muito melhor que vídeo nenhum.
        log.error("DB", `object_sizes indisponível key=${key}: ${err.message}`);
        return null;
    }
}
