...
// Estado da análise de proctoring (issue #220): { state:'running'|'failed', at, error? }
// em oral_voice_json.proctor_status. status=null limpa a chave (usado no sucesso —
// "concluído" é derivado de oral_proctor_json presente). Distingue "falhou/interrompida"
...
// Boot: análises que ficaram 'running' no banco são ÓRFÃS de um reinício do
// servidor (o processo que as rodava morreu) → viram 'failed'. Retorna o nº afetado.
export async function failOrphanProctorRunning() {
    const r = await pool.query(
        `UPDATE submissions
            SET oral_voice_json = oral_voice_json || jsonb_build_object('proctor_status',
                    jsonb_build_object('state','failed','error','interrompida por reinício do servidor'))
          WHERE oral_voice_json->'proctor_status'->>'state' = 'running'`
    );
    return r.rowCount;
}
