...
// A fonte dos alcances é o código; a tabela é reconciliada no boot
// (auth.js#seedTokenScopes) e conferida pelo health check (check `seeds`).
export const TOKEN_SCOPE_DEFS = [
    { key: "analytics", name: "Análise (benchmark)", ttl_days: 30 },
    { key: "health",    name: "Saúde (monitoração)", ttl_days: 365 },
];
...
export async function listAnalyticsTokens() {
    const { rows } = await pool.query(
        `SELECT t.id, t.token_prefix, t.label, t.created_by, t.scope, s.name AS scope_name,
         ...
         FROM analytics_tokens t LEFT JOIN analytics_token_scopes s ON s.key = t.scope
