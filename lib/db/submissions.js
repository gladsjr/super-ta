...
export async function listSubmissionsForWork(workId) {
    const r = await pool.query(`
      SELECT
        ...
        proctor_review,
        proctor_review_note,
        -- Avaliação PARCIAL (#362): quantas questões foram neutralizadas por
        -- nunca terem sido perguntadas (sessão interrompida). Serve para a
        -- listagem não confundir uma prova incompleta com uma completa.
        (grades_json->'partial'->>'not_asked')::int AS grade_not_asked,
