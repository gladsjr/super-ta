...
import { pump as pumpVideoLane } from "./proctorQueue.js";
import { markStarted, markTick, markTickError } from "./jobsHeartbeat.js";
...
export async function tick() {
    if (running) return;
    running = true;
    markTick(); // batimento lido pelo health check (#375)
    // Lane de VÍDEO (#338): bomba própria com política própria (concorrência
    // do dial, prioridade, sem retry automático) — o tique só a acorda, p/
    // jobs enfileirados por outro processo não apodrecerem na fila.
    pumpVideoLane();
    try {
        while (windowOpen()) {
