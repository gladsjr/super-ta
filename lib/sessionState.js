...
export function dehydrate(sess) {
    return {
        ...
        runtimeState: {
            ...
            // Bloco do super-orquestrador (experimento). work_analysis substitui
            // semanticamente o document_map antigo (que continua salvo acima por
            // compat com conversation.html e o path de rebuild). super_orchestrator
            // também vai carregar `memory` na fase 3 (estado interno do agente
            // entre turnos). Por ora só work_analysis.
            super_orchestrator: {
                version: 1,
                work_analysis: sess.workAnalysis ?? null,
            ...
            // Estado da fase de abertura. introStep guia o roteiro determinístico
            // (ask_name → present_self → begin); studentName é capturado no
            // present_self e reusado nas falas posteriores do entrevistador.
            intro_step: sess.introStep ?? null,
            student_name: sess.studentName ?? null,
            student_gender_hint: sess.studentGenderHint ?? null,
        },
    };
}
...
export function hydrate({ submission, work, runtimeRow, conversationLog }) {
    ...
    return {
        ...
        introStep: rs.intro_step ?? null,
        studentName: rs.student_name ?? null,
        studentGenderHint: rs.student_gender_hint ?? null,
        workAnalysis: rs.super_orchestrator?.work_analysis ?? null,
        superOrchestratorMemory: rs.super_orchestrator?.memory ?? null,
        introLog,
