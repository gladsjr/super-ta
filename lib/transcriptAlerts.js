...
export function detectTranscriptAlerts(transcript) {
    ...
    // turn_count = nº de turnos da transcrição analisada — âncora para a UI só
    // destacar se os alertas correspondem à transcrição ATUAL (#218).
    return { count: turns.length, turns, turn_count: (Array.isArray(transcript) ? transcript.length : 0) };
}
