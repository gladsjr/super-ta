...
// Cancela um item AINDA NA FILA (running não é interrompido). Marca 'failed' com
// motivo — limpar o status faria a reconciliação do boot re-enfileirar.
...
async function runItem(item) {
    ...
    try {
        ...
        await db.setOralProctorStatus(id, {
            state: "failed", at: new Date().toISOString(), error: e.message, attempts,
        }).catch(() => {});
        log.error("PROCTORQ", `análise falhou sub=${tok} attempts=${attempts}: ${e.message}`);
        item.reject(e);
    } finally {
        items.delete(id);
        pump();
