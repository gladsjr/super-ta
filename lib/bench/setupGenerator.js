...
function selectCanonical(proposals, votes, scenario) {
    ...
    const kindVotes = (field) => {
        const values = new Map();
        for (const vote of votes) for (const kind of vote[field] || []) values.set(kind, (values.get(kind) || 0) + 1);
        return [...values].filter(([, count]) => count >= threshold).map(([kind]) => kind).sort();
    };
    const acceptableKinds = kindVotes("acceptable_kinds");
    ...
    return {
        ...
        policy: {
            ...
            forbidden_kinds: kindVotes("forbidden_kinds").filter((kind) => !acceptableKinds.includes(kind)),
...
export async function generateCanonicalSetup({
    ...
    for (const testCase of cases) {
        ...
        generatedCases.push({ ...testCase, turns });
    }
    ...
    const result = {
        ...
        provenance: {
            ...
            cases_hash: sha256(generatedCases),
        },
        calls,
    };
