...
// Códigos de "arquivo momentaneamente travado" no Windows (Dropbox/antivírus
// abrem o arquivo logo após cada gravação; o rename em cima dele falha).
const FILE_LOCK_CODES = new Set(["EPERM", "EACCES", "EBUSY"]);

function atomicWriteJson(file, value) {
    const temporary = `${file}.tmp`;
    writeJson(temporary, value);
    // Retry curto: a janela de lock do sincronizador é de milissegundos.
    // O wait é bloqueante, mas raro e limitado (~175 ms no pior caso).
    for (let attempt = 0; ; attempt++) {
        try { fs.renameSync(temporary, file); return; }
        catch (err) {
            if (!FILE_LOCK_CODES.has(err.code) || attempt >= 3) throw err;
...
export async function runBenchmark(config, { dryRun = false, adapters = {}, onProgress = null, signal = null, casesOverride = null, resume = false } = {}) {
    ...
    // casesOverride é sempre CONGELADO (com turns — replay de release/teste): valida
    // como frozen. O caminho default carrega SEEDS (sem turns; os turns nascem ao
    // rodar), já validados como seed por loadCaseSeeds — não exigir turns aqui
    // (senão bench:validate/--dry-run e qualquer run pelo CLI quebram). Issue #106.
    const cases = casesOverride
        ? validateFrozenCases(casesOverride)
        : loadCaseSeeds(path.resolve(config.cases_dir), config.case_ids || []);
    ...
    let checkpointLockWarned = false;
    const saveCheckpoint = (phase = "running") => {
        ...
        try {
            ...
            // Checkpoint é resiliência, não resultado: se o arquivo seguir
            // travado (Dropbox/antivírus) mesmo após os retries, perder ESTE
            // save só faz o resume repetir um pouco — o próximo save (há um
            // por unidade concluída, mais o final) regrava o estado inteiro.
            // Derrubar a execução por causa disso é que seria perda real.
            if (!FILE_LOCK_CODES.has(err.code)) throw err;
            if (!checkpointLockWarned) {
                checkpointLockWarned = true;
                console.warn(`[BENCH] checkpoint adiado (${err.code} em rename): arquivo travado por outro processo (Dropbox/antivírus). Os próximos saves regravam o estado.`);
            }
        }
    };
    const advance = async () => {
    ...
    const run = { benchmark: config.benchmark, run_key: runKey, setup_version: config.setup_version, context_version: config.context_version, jury_version: config.jury_version, mode: config.mode, started_at: startedAt, finished_at: finishedAt, status: dryRun ? "validated" : "completed", cases: cases.length, turns: cases.reduce((sum, item) => sum + (item.turns?.length || 0), 0), candidates: candidates.map((item) => item.key), judges: judges.map((item) => item.key) };
