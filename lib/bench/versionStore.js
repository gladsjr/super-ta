...
// Lista SEM manifest_json: o manifesto (entrevistas congeladas) pode ter
// megabytes e a listagem só precisa dos metadados — o conteúdo vem pela
// rota de detalhe (getSetupVersion).
export async function listSetupVersions() {
    return (await pool.query(
        `SELECT version_key,version_major,version_minor,version_build,name,description,
                classification,source_generation_key,manifest_hash,published_by,published_at
         FROM benchmark_setup_versions ORDER BY version_major DESC,version_minor DESC,version_build DESC`
    )).rows;
}

export async function getSetupVersion(versionKey) {
    return (await pool.query("SELECT * FROM benchmark_setup_versions WHERE version_key=$1", [versionKey])).rows[0] || null;
}
