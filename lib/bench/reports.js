...
export function markdownReport(run, metrics) {
    const lines = [
        ...
        "",
        "| Modelo | Qualidade media | Mediana | Sinal e intensidade | Acao aceitavel | Acao preferida | Custo candidato | Latencia p50 | Latencia p95 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ];
    for (const [model, result] of Object.entries(metrics.models)) {
        ...
        lines.push(`| ${model} | ${n(q.mean)} | ${n(q.median)} | ${scoreSummary(q)} | ${percent(action.acceptable_rate)} | ${percent(action.preferred_rate)} | US$ ${n(cost.candidate_usd, 4)} | ${n(latency.p50, 0)} ms | ${n(latency.p95, 0)} ms |`);
    }
