import { ACTION_SCHEMA_DESCRIPTION } from "../superOrchestrator/actionSchema.js";

// Turnos NUMERADOS ([T0], [T1], ...): sem isso, about_turn_index é
// irrespondível — o modelo não tem como apontar um turno que não tem número.
// A numeração é a posição na lista do histórico; a fala corrente do aluno
// recebe o índice seguinte (ver candidatePrompt).
function historyText(history) {
    return history?.length ? history.map((item, index) => `[T${index}] ${item.role === "interviewer" ? "Entrevistador" : "Aluno"}: ${item.text}`).join("\n") : "(inicio da entrevista)";
}

// Contrato REAL de produção (schema anotado campo a campo) + as regras
// condicionais que o validador do benchmark cobra. Antes o candidato recebia
// uma frase seca e tinha de adivinhar a semântica dos campos pelo nome — o
// gabarito (conselho) ganhava retry com o erro na mão e o candidato não.
function contractText() {
    return `CONTRATO DE SAÍDA (o mesmo do sistema em produção):\n${ACTION_SCHEMA_DESCRIPTION}\nREGRAS CONDICIONAIS OBRIGATÓRIAS:\n- kind="follow_up" exige about_turn_index (o número [Tn] da fala do aluno referenciada) E follow_up_reason.\n- kind="finalize" exige finalize_reason.\n- kind="ask" espontânea (plan_question_id null) exige revisit_topic não-nulo.`;
}
...
export function setupVotePrompt(testCase, state, turn, evidenceText, proposals, setupPrompts = {}) {
    return `${caseContext(testCase, evidenceText)}\n\nESTADO CONGELADO:\n${historyText(turn.history)}\nAluno: ${turn.student_message}\n\nMEMÓRIA ANTERIOR:\n${JSON.stringify(turn.memory_before || {}, null, 2)}\n\nOBJETIVO, COMPORTAMENTO E CONTINUAÇÃO PLANEJADOS:\n${JSON.stringify(state, null, 2)}${versionedInstruction("RUBRICA VERSIONADA DE VALIDAÇÃO", setupPrompts.judge)}\n\nPROPOSTAS DO CONSELHO:\n${proposals.map((item) => `${item.id}:\n${JSON.stringify(item.output, null, 2)}`).join("\n\n")}\n\nEscolha a melhor proposta compatível com a trajetória. Marque todas as propostas e tipos de ação aceitáveis e os tipos proibidos neste estado. Uma referência canônica não precisa ser a única resposta correta: em acceptable_kinds, inclua TODO tipo de ação pedagogicamente defensável neste estado (mesmo que não seja o da proposta escolhida — ex.: ask e follow_up costumam ser ambos defensáveis num mesmo estado); em forbidden_kinds, apenas os tipos que seriam um ERRO pedagógico aqui. Aceitável ≠ preferido.`;
}
