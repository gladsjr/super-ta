...
export function summarizeJudgments(items) {
    ...
    // Taxa calculada só sobre avaliações que TÊM o campo: runs antigos
    // (importados antes da decomposição executável/conforme/adequada) não o
    // trazem, e 0% seria mentira — null vira "—" na interface.
    const rate = (field) => {
        const withField = assessments.filter((item) => field in item);
        return withField.length ? withField.filter((item) => item[field]).length / withField.length : null;
    };
    const confusion = {};
    ...
    return {
        ...
        action_policy: {
            ...
            compliant_rate: rate("compliant"),
            adequate_rate: rate("adequate"),
