...
// unitId é OBRIGATÓRIO e escopa a exclusão à unidade da rota — sem ele, um admin
// da unidade A com um membershipId da unidade B apagaria o vínculo alheio (IDOR
// horizontal, issue #141). A autorização por unidade já foi feita no handler.
export async function removeMembership(id, unitId) {
    const r = await pool.query(`DELETE FROM memberships WHERE id = $1 AND unit_id = $2 RETURNING id`, [id, unitId]);
