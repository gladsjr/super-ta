...
export function buildAuditBlock({ final, multiPart = false }) {
    ...
    if (final.mode === "answers" && Array.isArray(final.answers)) {
        ...
        for (const a of final.answers) {
            ...
            const turno = Number.isInteger(a.turn_index) ? `turno ${a.turn_index}` : `áudio ${a.audio_idx}`;
...
export function auditPromptBlock(audit) {
    ...
    const attrib = audit.mode === "segments"
        ...
        : audit.mode === "answers"
        ? "Cada resposta traz o TURNO da conversa a que pertence (fronteira exata do áudio gravado)."
