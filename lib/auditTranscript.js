...
export function buildAuditBlock({ final, multiPart = false, humanLabels = false }) {
    ...
    if (final.mode === "answers" && Array.isArray(final.answers)) {
        ...
        for (const a of final.answers) {
            ...
            // Resposta que NÃO foi retranscrita vira MARCA, não omissão (#359).
            // Omiti-la deixava o bloco com cara de completo — e o prompt manda
            // confiar nele como fonte de maior fidelidade, de modo que o
            // avaliador concluía que o aluno não disse nada naquele turno.
            if (a.error || !a.text) {
                lacunas++;
                lines.push(`[${turno}${iv}] (esta resposta não pôde ser retranscrita — use a transcrição ao vivo para ela)`);
                continue;
            }
...
export function auditPromptBlock(audit) {
    ...
    // Bloco INCOMPLETO precisa se anunciar (#359): sem isto, a instrução de
    // "confie neste bloco" faz o avaliador ler ausência como silêncio do aluno.
    const lacunas = audit.lacunas > 0
        ? ` ATENÇÃO: ${audit.lacunas} resposta(s) NÃO puderam ser retranscritas e aparecem marcadas abaixo — nesses pontos este bloco NÃO é a fonte melhor, e você deve usar a transcrição ao vivo. Ausência de texto aqui NÃO significa que o aluno ficou calado.`
        : "";
    return `

**RETRANSCRIÇÃO DE AUDITORIA (fala do aluno — fonte de maior fidelidade)**
A transcrição da conversa acima foi gerada AO VIVO e pode conter falas do aluno picotadas, trocadas ou corrompidas pela captação. O bloco abaixo foi retranscrito do áudio contínuo da sessão, depois dela, e é mais fiel às PALAVRAS DO ALUNO (só a voz dele — as falas do examinador não aparecem aqui). Quando as duas fontes divergirem sobre o que o aluno DISSE, confie neste bloco. ${attrib}${lacunas}
${audit.text}`;
}
