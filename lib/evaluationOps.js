...
import { renderProctorReviewForPrompt } from "./proctorReview.js";
...
export async function evaluateSubmissionNow(work, found, { force }) {
    ...
    // Corte 4 (#289): na variante realtime, a retranscrição de auditoria do
    // áudio contínuo entra como fonte de maior fidelidade da fala do aluno.
    // Sessão retomada (multi-parte) → o tee cobre só o último trecho: o bloco
    // cai para o texto contínuo (sem atribuição posicional). Ausente = "".
    let auditBlock = "";
    if (conversation?.channel === "realtime_voice") {
        try {
            const [final, detail] = await Promise.all([
                db.getFinalTranscript(found.id),
            ...
            auditBlock = auditPromptBlock(audit);
        } catch (err) {
...
export async function deriveStudentVersionNow(work, found, { force, guidelinesOverride }) {
    ...
    if (work.proctoring_enabled && review?.level !== "sem_problema") {
        const d = await db.getOralSubmissionDetail(found.id);
        proctoringSummary = renderProctorForStudent(d && d.oral_proctor_json);
    }
    const reviewBlock = renderProctorReviewForPrompt(review, { paraDevolutiva: true });
    if (reviewBlock) proctoringSummary = [reviewBlock, proctoringSummary].filter(Boolean).join("\n\n");
