...
// — mas o reprocessado vira CACHED (barato), o que mitiga. Preços oficiais em
// config/pricing.yaml (realtime.gpt-realtime).
export async function recordRealtimeCost({ workId, submissionId = null, model, usages = [] }) {
    ...
    log.info("BILLING", `realtime[oral] model=${model} inAudio=${inAudio} inText=${inText} cached=${inCached} outAudio=${outAudio} outText=${outText} responses=${usages.length} cost=$${cost.toFixed(6)}`);
    await recordCost({
