...
export function computeSttCost(usage, model) {
    ...
    if (usage?.type === "tokens") {
        // Modelos por-duração (gpt-transcribe) não têm preço por token
        // publicado; se a API mudar o formato do usage, falhar explícito é
        // melhor que registrar NaN/zero em silêncio (lição do metering
        // realtime zerado).
        if (p.input_per_mtok == null || p.output_per_mtok == null) {
            throw new Error(`pricing.stt.${model}: usage veio por tokens mas o pricing só tem per_second_usd`);
        }
        const input = Number(usage.input_tokens ?? 0);
        const output = Number(usage.output_tokens ?? 0);
        const cost =
...
// lib/realtimeBackfill.js, removido após o backfill único de 26/07/2026 —
// ver histórico do git). As frações de áudio abaixo foram CALIBRADAS na
...
// ---------------------------------------------------------------------------
// Gate A — orçamento US$ por UNIDADE como RESERVA (mesmo modelo mental do
// Portão B, decisão de 04/08/2026). O teto de uma filha é um PEDAÇO do saldo
// do pai, debitado no ato em que é definido/aumentado; reduzir devolve. O que
// uma unidade COMPROMETE do próprio teto = gasto direto dos seus trabalhos
// + a reserva (teto) de cada filho com teto próprio + o comprometido recursivo
// dos filhos SEM teto (que consomem do saldo dela). O gasto dentro de um filho
// com teto NÃO conta de novo no pai — já está coberto pela reserva.
//
// O gasto de um trabalho segue debitando works.spent_usd (recordCost,
// inalterado). Trabalhos legados (unit_id NULL) não entram em nenhuma
// sub-árvore — só batem no teto por-trabalho.
// ---------------------------------------------------------------------------

// Sub-árvore com orçamento e gasto direto por unidade, numa query só.
async function fetchBudgetSubtree(q, unitId) {
    const r = await q.query(
        `WITH RECURSIVE down AS (
             SELECT id, parent_id, budget_usd FROM units WHERE id = $1
             UNION ALL
             SELECT u.id, u.parent_id, u.budget_usd
               FROM units u JOIN down d ON u.parent_id = d.id
         )
         SELECT d.id, d.parent_id, d.budget_usd::float8 AS budget_usd,
                COALESCE(w.spent, 0)::float8 AS direct_spent
           FROM down d
           LEFT JOIN (
               SELECT unit_id, SUM(spent_usd) AS spent
                 FROM works WHERE unit_id IS NOT NULL GROUP BY unit_id
           ) w ON w.unit_id = d.id`,
        [unitId]
    );
    return r.rows;
...
// Comprometido da raiz da sub-árvore carregada. `overrides` (Map id→budget)
// simula um teto diferente numa unidade — usado para validar uma reserva nova
// antes de gravá-la (override null = tratar como sem teto próprio).
function computeCommitted(rows, rootId, overrides = new Map()) {
    const byId = new Map(rows.map((r) => [r.id, r]));
    const kids = new Map();
    for (const row of rows) {
        if (row.parent_id != null && byId.has(row.parent_id)) {
            if (!kids.has(row.parent_id)) kids.set(row.parent_id, []);
            kids.get(row.parent_id).push(row.id);
        }
    }
    const walk = (id) => {
        let total = byId.get(id)?.direct_spent ?? 0;
        for (const kid of kids.get(id) ?? []) {
            const b = overrides.has(kid) ? overrides.get(kid) : byId.get(kid)?.budget_usd;
            total += b != null ? b : walk(kid);
        }
        return total;
    };
    return walk(rootId);
...
export async function getUnitCommitted(unitId, q = pool) {
    const rows = await fetchBudgetSubtree(q, unitId);
    if (!rows.length) return null;
    return computeCommitted(rows, unitId);
}

// Saldo de uma unidade. committed = o que já está reservado/comprometido do
// teto (ver acima); spent = gasto REAL consolidado da sub-árvore (informativo).
// budget_usd NULL = sem teto próprio (consome do ancestral; remaining null).
export async function getUnitBalance(unitId) {
    const rows = await fetchBudgetSubtree(pool, unitId);
    if (!rows.length) return null;
    const budget = rows.find((r) => r.id === unitId)?.budget_usd ?? null;
    const committed = computeCommitted(rows, unitId);
    const spent = rows.reduce((acc, r) => acc + r.direct_spent, 0);
    const remaining = budget == null ? null : Math.max(0, budget - committed);
    const percent_used = budget == null ? null : (budget > 0 ? Math.min(100, (committed / budget) * 100) : 100);
    return { budget_usd: budget, committed_usd: committed, spent_usd: spent, remaining_usd: remaining, percent_used };
}

// Define o teto de uma unidade RESERVANDO contra o saldo do ancestral
// vinculante mais próximo (transação; trava a cadeia para serializar reservas
// de irmãs). Regras:
// - aumentar/definir: o delta precisa caber no saldo do ancestral com teto
//   (se nenhum ancestral tem teto, é alocação livre de topo);
// - reduzir: o novo teto não pode ficar abaixo do já comprometido da própria
//   unidade (piso); a diferença volta ao saldo do pai por construção;
// - remover (null): a unidade volta a consumir do saldo do ancestral; como
//   comprometido ≤ teto, remover nunca estoura o pai.
export async function setUnitBudgetReserved(unitId, budgetUsd) {
    const b = budgetUsd == null ? null : Number(budgetUsd);
    ...
    try {
        ...
        const chainRows = await client.query(
            `WITH RECURSIVE up AS (
                 SELECT id, parent_id, budget_usd, 0 AS depth FROM units WHERE id = $1
                 UNION ALL
                 SELECT u.id, u.parent_id, u.budget_usd, up.depth + 1
                   FROM units u JOIN up ON u.id = up.parent_id
             )
             SELECT id, budget_usd::float8 AS budget_usd, depth FROM up ORDER BY depth ASC`,
            [unitId]
        );
        if (chainRows.rowCount === 0) throw Object.assign(new Error("not_found"), { status: 404 });
        const chain = chainRows.rows;
        // Trava as unidades da cadeia em ordem estável (evita deadlock entre
        // reservas concorrentes) — irmãs disputando o mesmo pai serializam aqui.
        await client.query(
            `SELECT id FROM units WHERE id = ANY($1::int[]) ORDER BY id FOR UPDATE`,
            [chain.map((c) => c.id)]
        );
        if (b != null) {
            const own = await getUnitCommitted(unitId, client);
            if (b < own) {
                throw Object.assign(new Error("budget_below_committed"), { status: 409, committed: own });
            }
        }
        const binding = chain.slice(1).find((c) => c.budget_usd != null) || null;
        if (binding && b != null) {
            const rows = await fetchBudgetSubtree(client, binding.id);
            const committed = computeCommitted(rows, binding.id, new Map([[unitId, b]]));
            if (committed > binding.budget_usd) {
                throw Object.assign(new Error("insufficient_parent_balance"), {
                    status: 409, parent_unit_id: binding.id,
                });
            }
        }
...
// o COMPROMETIDO da sua sub-árvore vs o teto. Basta um vinculante estourado
...
export async function isUnitCeilingExceeded(unitId) {
    ...
    const r = await pool.query(
        `SELECT id, budget_usd::float8 AS budget_usd
           FROM units WHERE id = ANY($1::int[]) AND budget_usd IS NOT NULL`,
        [chain]
    );
    for (const row of r.rows) {
        const committed = await getUnitCommitted(row.id);
        if (committed != null && committed >= row.budget_usd) return true;
    }
    return false;
}
