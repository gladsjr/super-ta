...
// Realtime (prova oral): grava o custo da SESSÃO a partir do usage acumulado
// dos eventos response.done. Cada usage traz tokens por modalidade (áudio/texto)
// e direção. Somamos a sessão e gravamos UM evento. Caveat: o input do realtime
// reprocessa o contexto a cada resposta, então a soma pode superestimar o input
// — mas o reprocessado vira CACHED (barato), o que mitiga. Os TOKENS são reais;
// o $ usa pricing PLACEHOLDER até confirmarmos a tabela oficial.
export async function recordRealtimeCost({ workId, submissionId = null, model, usages = [] }) {
    ...
    let inAudio = 0, inText = 0, inCached = 0, outAudio = 0, outText = 0;
    for (const u of usages) {
        const i = u?.input_token_details || u?.input_tokens_details || {};
        const o = u?.output_token_details || u?.output_tokens_details || {};
        inAudio += Number(i.audio_tokens || 0);
        inText += Number(i.text_tokens || 0);
        const cd = i.cached_tokens_details || {};
        inCached += Number(i.cached_tokens ?? ((cd.audio_tokens || 0) + (cd.text_tokens || 0)) ?? 0);
    ...
    if (p) {
        cost = (
            Math.max(0, inText) * p.text_input_per_mtok +
            Math.max(0, inAudio) * p.audio_input_per_mtok +
            Math.max(0, inCached) * p.cached_per_mtok +
    ...
    log.info("BILLING", `realtime[oral] model=${model} inAudio=${inAudio} inText=${inText} cached=${inCached} outAudio=${outAudio} outText=${outText} responses=${usages.length} cost=$${cost.toFixed(6)} (PLACEHOLDER pricing)`);
    await recordCost({
        workId, submissionId, eventType: "realtime", model, agentLabel: "AGENT:OralRealtime",
        inputTokens: inAudio + inText, cachedTokens: inCached, outputTokens: outAudio + outText, costUsd: cost,
    });
}
