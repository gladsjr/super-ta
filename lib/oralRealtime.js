import { isRealtimeVoice, FALLBACK_VOICE } from "../config/voices.js";
...
import { runRetranscribeAuto } from "./retranscribe.js";
...
function buildBridgeConfig({ found, questions, priorAsked, priorTranscript }) {
    ...
    return {
        ...
        // #351: voz incompatível com Realtime cai no fallback — a alternativa é a
        // OpenAI recusar a configuração toda e a prova rodar com o agente padrão.
        voice: isRealtimeVoice(found.work_voice) ? found.work_voice : FALLBACK_VOICE,
        totalQuestions: n,
        resuming,
        ...
        // TEE + retranscrição (#289): guarda a referência do áudio contínuo e
        // dispara a retranscrição de auditoria em background (transcript_final,
        // em CONVIVÊNCIA — não substitui a fonte da avaliação nesta fase).
        onTeeAudio: async (teeResult) => {
            await db.setOralVoice(found.id, { tee: { key: teeResult.key, duration_s: teeResult.duration_s, index: teeResult.index } });
            runRetranscribeAuto({
                submissionId: found.id, token, oggBuffer: teeResult.ogg, tee: teeResult,
                questions: sampled, workName: found.work_name,
                isBenchmark: !!found.work_is_benchmark,
                meterCtx: { workId: found.work_id, submissionId: found.id },
            });
        },
    };
}
