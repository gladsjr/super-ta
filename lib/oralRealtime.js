...
function bridge(clientWs, { found, questions, priorAsked, priorTranscript }) {
    ...
    function nudgeSilence(stage) {
        ...
        const note = stage >= 2
            ...
            : `SITUAÇÃO: o aluno está EM SILÊNCIO e NÃO respondeu nada${absent ? "; parece ter saído do enquadramento da câmera" : ""}. NÃO invente, NÃO presuma e NÃO comente nenhuma resposta dele. Apenas verifique GENTILMENTE, na sua voz de examinador, se está tudo bem e se ele consegue continuar — sem repetir a pergunta inteira (a menos que ele peça).`;
        try {
    ...
    oai.on("open", () => {
        ...
        watchdog = setInterval(() => {
            ...
            // O silêncio conta do FIM da fala do examinador (início do turno do aluno),
            // não da abertura da sessão — senão a introdução longa já estoura o limiar
            // e dispara uma cutucada-fantasma logo na 1ª pergunta.
            const quiet = Date.now() - Math.max(lastVoiceAt, examinerDoneAt);
