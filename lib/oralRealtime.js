...
- Depois da resposta à ÚLTIMA pergunta, chame IMEDIATAMENTE a ferramenta "encerrar_prova". NÃO precisa se despedir você mesmo — o SISTEMA cuida da despedida e avisa o aluno para aguardar o envio do vídeo. Depois de encerrar, NÃO faça mais nenhuma pergunta nem volte a falar.
...
const END_EXAM_TOOL = {
    ...
    description:
        ...
        "pergunta da lista. O SISTEMA cuida da despedida — você não precisa se " +
        "despedir antes. Nunca chame antes de cobrir todas as perguntas. Sem parâmetros.",
    parameters: { type: "object", properties: {}, required: [] },
};

// Fala de ENCERRAMENTO, disparada pelo SERVIDOR quando a ferramenta é aceita
// (garante a despedida mesmo que o modelo chame a ferramenta sem falar). Só a
// despedida — nada de novas perguntas.
const FAREWELL_INSTR = `A prova acabou. Diga APENAS uma despedida curta e cordial, em português do Brasil com pronúncia brasileira natural: agradeça em uma frase e peça para o aluno AGUARDAR alguns instantes enquanto o vídeo é enviado (ex.: "Era isso, muito obrigado — sua prova está encerrada. Só aguarde alguns instantes enquanto o vídeo é enviado."). NÃO faça mais nenhuma pergunta nem comente respostas.`;
    ...
    return {
        ...
        audio: {
            input: {
                ...
                // VAD por SILÊNCIO (server_vad), não semântico: o semantic_vad
                // (classificador de fim-de-turno) mostrou-se pouco confiável em
                // português no gpt-realtime-2.1 — cortava a ÚLTIMA palavra da resposta
                // (ex.: "...e segurança"). O server_vad é agnóstico de idioma: só fecha
                // o turno após SILENCE_MS de silêncio, então captura o fim da fala.
                // Janela generosa (1s) p/ tolerar micro-pausas antes da última palavra;
                // pausas longas de "pensar" seguem cobertas pelo vigia de silêncio.
                // interrupt_response: o examinador PARA de falar quando o aluno começa (barge-in).
                turn_detection: { type: "server_vad", threshold: 0.5, prefix_padding_ms: 300, silence_duration_ms: 1000, interrupt_response: true, create_response: true },
            },
...
function bridge(clientWs, { found, questions, priorAsked, priorTranscript }) {
    ...
    let farewellState = null; // null | 'requested' (tool aceita) | 'awaiting' (despedida em geração) | 'done'
    let farewellTimer = null; // fallback: manda wrap-up mesmo se a despedida não gerar
    ...
    function handleEndTool(callId) {
        ...
        // Despedida DETERMINÍSTICA pelo servidor: o gpt-realtime-2.1 às vezes chama a
        // ferramenta SEM falar a despedida. Em vez de confiar no modelo, disparamos
        // NÓS a fala de encerramento e só mandamos 'wrapup' quando ela terminar de
        // gerar (aí o áudio já está agendado no cliente, que espera drenar). A
        // sequência de response.done é tratada em handleFarewellOnDone().
        farewellState = "requested";
    }
    // Aciona/finaliza a despedida seguindo os response.done. Chamado no case response.done.
    function handleFarewellOnDone() {
        if (farewellState === "requested") {
            // Este done é o da resposta que continha a chamada da ferramenta. Agora
            // criamos a resposta da DESPEDIDA (uma nova fala do examinador).
            farewellState = "awaiting";
            try { oai.send(JSON.stringify({ type: "response.create", response: { instructions: FAREWELL_INSTR } })); }
            catch (err) { log.error("ORAL_RELAY", `farewell create submission=${token}: ${err.message}`); farewellState = "done"; sendClient({ type: "wrapup" }); return; }
            // Fallback: se a despedida não gerar em ~8s, encerra mesmo assim (não trava a prova).
            farewellTimer = setTimeout(() => { if (farewellState !== "done") { farewellState = "done"; log.warn("ORAL_RELAY", `farewell timeout submission=${token}`); sendClient({ type: "wrapup" }); } }, 8000);
        } else if (farewellState === "awaiting") {
            // Done da despedida: o áudio já foi transmitido. Agora sim, wrap-up.
            farewellState = "done";
    ...
    oai.on("message", (data, isBinary) => {
        ...
        switch (m.type) {
            ...
            case "conversation.item.input_audio_transcription.completed":
                if (m.transcript) { transcript.push({ role: "student", text: m.transcript }); lat.studentAnswer(m.transcript); studentTurns++; silenceNudges = 0; }
                break;
            case "response.created":
                examinerSpeaking = true; break; // examinador começou a responder → pausa o vigia
            ...
            case "response.done":
                ...
                if (farewellState === "requested" || farewellState === "awaiting") handleFarewellOnDone();
                break;
            case "error":
