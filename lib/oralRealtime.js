...
function bridge(clientWs, { found, questions }) {
    ...
    oai.on("message", (data, isBinary) => {
        ...
        switch (m.type) {
            ...
            case "conversation.item.input_audio_transcription.completed":
                if (m.transcript) transcript.push({ role: "student", text: m.transcript });
                break;
            case "response.done":
    ...
    clientWs.on("message", (data, isBinary) => {
        ...
        if (isBinary) {
            ...
            let m; try { m = JSON.parse(data.toString()); } catch { return; }
            if (m.type === "bye") { try { oai.close(); } catch {} }
        }
    });
    ...
    clientWs.on("close", async () => {
        ...
        // No-retake: se a prova de fato rodou (o examinador falou ao menos uma
        // vez), marca como concluída para impedir refazer. Idempotente.
        if (usageAcc.length > 0) {
        ...
        log.info("ORAL_RELAY", `bridge closed submission=${token} responses=${usageAcc.length}`);
    });
}
