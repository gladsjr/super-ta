...
import { openai, clientForWork, apiKeyForWork } from "./openaiClient.js";
...
function bridge(clientWs, { found, questions, priorAsked, priorTranscript }) {
    ...
    // Benchmark de custo: roteia o WebSocket do Realtime e a despedida (TTS) pela
    // chave dedicada quando o work é de benchmark. Ver lib/openaiClient.js.
    const workRef = { is_benchmark: !!found.work_is_benchmark };
    ...
    async function speakTtsFarewell() {
        ...
        for (const v of [voice, "verse"]) {
            try {
                const pcm = await synthesizeSpeech(clientForWork(workRef), TTS_MODEL, FAREWELL_TTS_TEXT, v, "pcm");
