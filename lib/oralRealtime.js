...
const SILENCE_T1_PRESENT_MS = 20000; // presente e calado → checagem gentil
const SILENCE_T1_ABSENT_MS = 8000;   // fora do quadro → mais rápido
const SILENCE_T2_MS = 25000;         // ainda calado após a checagem → escala (pausa)
...
function bridge(clientWs, { found, questions, priorAsked, priorTranscript }) {
    ...
    function nudgeSilence(stage) {
        ...
        const note = stage >= 2
            ? "SITUAÇÃO: o aluno segue em silêncio mesmo após você verificar. Diga, de forma tranquila e BREVE, que vai PAUSAR aqui e que ele pode RECARREGAR a página quando puder para continuar de onde parou."
            : `SITUAÇÃO: faz um tempo que o aluno está em silêncio${absent ? " e parece ter saído do enquadramento da câmera" : ""}. Se ele acabou de responder algo, comente e prossiga normalmente. Se NÃO houve resposta, verifique GENTILMENTE, na sua voz de examinador, se está tudo bem e se ele consegue continuar — sem pressionar e sem repetir a pergunta inteira (a menos que ele peça).`;
    ...
    oai.on("open", () => {
        ...
        watchdog = setInterval(() => {
            ...
            const t1 = studentPresent ? SILENCE_T1_PRESENT_MS : SILENCE_T1_ABSENT_MS;
            if (silenceNudges === 0 && quiet > t1) { silenceNudges = 1; nudgeSilence(1); }
            else if (silenceNudges === 1 && quiet > t1 + SILENCE_T2_MS) { silenceNudges = 2; nudgeSilence(2); }
        }, SILENCE_POLL_MS);
        watchdog.unref?.();
    });
    ...
    oai.on("message", (data, isBinary) => {
        ...
        switch (m.type) {
            ...
            case "response.done":
                ...
                examinerSpeaking = false; examStarted = true; // terminou → começa a contar o silêncio
                break;
            case "error":
    ...
    clientWs.on("message", (data, isBinary) => {
        ...
        if (isBinary) {
            ...
            else if (m.type === "done_speaking") {
                // Válvula manual: o aluno sinaliza que terminou de responder. Força o
                // fecho do turno (commit do buffer + pedir resposta) para o caso de o
                // VAD semântico "esperar demais" e travar — a prova não fica pendurada.
                // [validar no publicado: se o VAD ativo rejeitar o commit, o catch segura.]
                try {
                    oai.send(JSON.stringify({ type: "input_audio_buffer.commit" }));
                    oai.send(JSON.stringify({ type: "response.create" }));
                } catch (err) { log.error("ORAL_RELAY", `done_speaking submission=${token}: ${err.message}`); }
            }
        }
    });
