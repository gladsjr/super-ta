...
import { openai, clientForWork } from "./openaiClient.js";
...
import { resumeDecision, RESUME_FRESH, RESUME_LEGACY } from "./interviewResume.js";
import {
...
// Varredura de sessões FINALIZADAS. Elas não podem ser apagadas na hora do
// finalize (o áudio de despedida é servido do sess.audioCache logo depois), então
// acumulam no mapa — vazamento lento de memória. Aqui removemos as que já
// finalizaram há mais de FINALIZED_TTL_MS (tempo de sobra p/ o cliente buscar o
// último áudio). A varredura NUNCA toca em sessões ativas (conversationCompleted
// !== true). A marcação de tempo (finalizedAt) é feita pela própria varredura na
// 1ª vez que vê a sessão concluída — sem depender de editar os pontos de finalize.
const FINALIZED_TTL_MS = 10 * 60 * 1000; // 10 min após finalizar
const SESSION_SWEEP_MS = 5 * 60 * 1000;  // roda a cada 5 min
const _sessionSweep = setInterval(() => {
    const now = Date.now();
    for (const [token, sess] of SESSIONS) {
        if (!sess || sess.conversationCompleted !== true) continue;
        if (sess.finalizedAt == null) { sess.finalizedAt = now; continue; } // 1ª aparição: marca
        if (now - sess.finalizedAt > FINALIZED_TTL_MS) {
            try { sess.audioCache?.clear?.(); } catch { /* best-effort */ }
            SESSIONS.delete(token);
            log.info("SESSION_SWEEP", `sessão finalizada removida da memória token=${token}`);
        }
    }
}, SESSION_SWEEP_MS);
_sessionSweep.unref?.(); // não segura o processo vivo (testes, shutdown limpo)
...
export function sessionMeterCtx(sess) {
    if (!sess) return null;
    // `openai`: cliente por-work (benchmark usa a chave dedicada). Os agentes
    // resolvem meterCtx.openai ?? this.client. Fallback ao singleton de produção
    // para qualquer sess construída sem o campo. Ver lib/openaiClient.js.
    return { workId: sess.workId ?? null, submissionId: sess.submissionId ?? null, openai: sess.openai || openai };
}
...
export async function createVectorStoreWithFiles(fileIds, sessionId, client = openai) {
...
// Quebra a fala do entrevistador em pedaços para TTS incremental. O 1º pedaço é
// a 1ª sentença sozinha (toca cedo → menor latência percebida); os demais
// agrupam sentenças até ~140 chars (menos chamadas de TTS). O custo por
// caractere é o mesmo do blob único; muda só o número de chamadas.
function splitForSpeech(text) {
    ...
    for (let i = 1; i < sentences.length; i++) {
        ...
        if (buf.length >= 140) { chunks.push(buf); buf = ""; }
    }
    if (buf) chunks.push(buf);
    return chunks;
}

// Variante fragmentada de attachAudio (modo áudio): sintetiza a fala por
// sentenças e entrega cada pedaço via onChunk({audio_url, index, is_last})
// assim que pronto, para o frontend tocar a 1ª sentença sem esperar o resto.
// Cada pedaço é cacheado com turnId composto "<base>.<i>" e servido pela mesma
// rota GET /audio/:turnId. Billing por pedaço via meteredTts (custo/caractere
// idêntico ao blob).
//
// Robustez (invariantes que NÃO podem quebrar): fragmentar multiplica o número
// de chamadas de TTS — logo, a chance de uma falhar. Duas garantias:
//   (1) NUNCA descartar conteúdo: se um pedaço falha, o restante (que pode conter
//       a própria pergunta) é sintetizado num único blob de fallback. O caractere
//       só é cobrado uma vez (não re-sintetiza o que já passou).
//   (2) SEMPRE terminar: exatamente um pedaço emitido carrega is_last=true, senão
//       o player do aluno nunca recebe o sinal de fim e a vez dele trava.
// Só degrada a texto (audio_error) se nem o 1º pedaço nem o fallback saírem.
export async function attachAudioChunks(sess, content, onChunk) {
    ...
    const parts = splitForSpeech(content);
    if (!parts.length) return {};
    const base = String(sess.audioTurnIdCounter++);
    let firstUrl = null;
    let totalDur = 0;
    let emitted = 0;

    // Sintetiza um texto com 1 retry (erros de TTS são tipicamente transitórios).
    // Devolve Buffer, ou null se falhar nas duas tentativas.
    const synth = async (text, label) => {
        for (let attempt = 1; attempt <= 2; attempt++) {
            try {
                return await meteredTts(
                    { ...sessionMeterCtx(sess), model: TTS_MODEL, inputText: text },
                    () => synthesizeSpeech(sess.openai || openai, TTS_MODEL, text, sess.voice)
                );
            } catch (err) {
                log.error("AUDIO", `TTS ${label} tentativa ${attempt}/2 falhou: ${err.message}`);
            }
        }
        return null;
    };

    // Emite um pedaço já sintetizado: cacheia, monta URL, dispara onChunk.
    const emit = (buffer, isLast) => {
        const turnId = `${base}.${emitted}`;
        sess.audioCache.set(turnId, buffer);
        const url = `/s/${encodeURIComponent(sess.submissionToken)}/audio/${turnId}`;
        if (emitted === 0) firstUrl = url;
        if (typeof onChunk === "function") {
            onChunk({ audio_url: url, index: emitted, is_last: isLast });
        }
        emitted++;
    };
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        let buffer = await synth(parts[i], `pedaço ${i}`);
        if (!buffer) {
            // Falha definitiva neste pedaço. NUNCA fazer break aqui: descartaria o
            // restante (que pode ser a pergunta) e nunca emitiria is_last, travando
            // a vez do aluno — foi exatamente esse o bug. Fallback: sintetiza TODO o
            // restante (parts[i..]) num único blob, emitido como o ÚLTIMO pedaço.
            const rest = parts.slice(i).join(" ");
            buffer = await synth(rest, `restante a partir de ${i} (blob)`);
            if (!buffer) {
                // Catástrofe (TTS falhou 4x seguidas). Sem nada emitido, degrada a
                // texto. Com algo já emitido, sinaliza fim (is_last) num pedaço
                // vazio para a vez do aluno não travar — fala incompleta é ruim,
                // mas travar para sempre é pior. O frontend trata audio_url nulo.
                log.error("AUDIO", `TTS impossível a partir do pedaço ${i} (${emitted} já emitido[s])`);
                if (emitted === 0) return { audio_error: "tts_failed" };
                if (typeof onChunk === "function") {
                    onChunk({ audio_url: null, index: emitted, is_last: true });
                }
                return { audio_url: firstUrl, audio_duration_seconds: totalDur || null, chunk_count: emitted, audio_error: "tts_partial" };
            }
            const dur = await getAudioDurationSeconds(buffer, "audio/mpeg");
            if (dur) totalDur += dur;
            emit(buffer, true);
            break; // o blob de fallback cobriu todo o restante
        }
        const dur = await getAudioDurationSeconds(buffer, "audio/mpeg");
        if (dur) totalDur += dur;
        emit(buffer, isLastPart);
    }
    // Re-entry (reload/restart): aponta para o 1º pedaço. Limitação aceita — na
    // recomposição o aluno ouve ao menos o início; fragmentar o re-entry fica
    // fora do escopo deste experimento.
    sess.lastInterviewerAudio = { turnId: `${base}.0`, durationSec: null };
    return { audio_url: firstUrl, audio_duration_seconds: totalDur || null, chunk_count: emitted };
}
...
export async function attachAudio(sess, content) {
    ...
    try {
        const buffer = await meteredTts(
            ...
            () => synthesizeSpeech(sess.openai || openai, TTS_MODEL, content, sess.voice)
        );
...
export async function maybeRebuildPendingQuestionAudio(sess) {
    ...
    // Só há "pergunta pendente" para tocar na re-entrada em intro/interviewing.
    // Em awaiting_upload a saudação pode JÁ estar pré-gerada (e
    // sess.lastInterviewerAudio apontando para ela como efeito colateral do
    // attachAudio na pré-geração), mas ela só é ENTREGUE depois do /upload — não
    // pode tocar antes do envio do PDF e do consentimento. Sem esta guarda, o
    // fast-path abaixo vazaria a saudação pré-gerada na 2ª entrada (após a
    // pré-geração terminar), pulando upload e LGPD.
    if (sess.currentPhase !== "intro" && sess.currentPhase !== "interviewing") return null;
...
export function startInterviewPreparation(sess) {
    ...
    sess.interviewPreparation = (async () => {
        try {
            ...
            // Files + VectorStore vão pelo cliente por-work: um work de benchmark
            // cria esses recursos no SEU projeto, para que o PrepBuilder e o
            // SuperOrchestrator (mesma chave) consigam anexá-los/consultá-los.
            // Cross-project daria 404. Ver lib/openaiClient.js.
            const cli = sess.openai || openai;
            ...
            // por PrepBuilder.buildPrep — análise + plano numa ÚNICA chamada
            // (não reenvia os PDFs; ~metade do custo/tempo da prep), com
            // fallback automático para as duas chamadas serializadas
            // (analyze → plan) se a chamada única vier incompleta/inválida.
            // Em paralelo com a prep, criamos o vector store com OS DOIS PDFs
            // (aluno + enunciado) — extensão prevista no plano para o
            // super-orquestrador poder cruzar "o que foi pedido vs entregue".
            //
            // Latência da prep cabe na janela do intro (que roda paralelo com
            // ela). Ver docs/super-orchestrator-plan.md.
            log.info("PREP", "step=parallel(vector_store,prep) start");
            const stage1 = await Promise.allSettled([
                createVectorStoreWithFiles([studentFile.id, enunciadoFile.id], params.token, cli),
                prepBuilderAgent.buildPrep({
            ...
            const stage1Labels = ["vector_store", "prep"];
            ...
            const [vsRes, prepRes] = stage1;
            sess.vectorStoreId = vsRes.value;
            sess.workAnalysis = prepRes.value.analysis;
            sess.interviewPlan = prepRes.value.plan;
            ...
            try {
                await cli.conversations.items.create(sess.conversationId_eval, {
                    items: [{ role: "developer", content: `[WORK_ANALYSIS] ${JSON.stringify(sess.workAnalysis, null, 2)}` }],
                });
                await logLastConvItem(sess.conversationId_eval, "CONV:eval", cli);
            } catch (err) {
...
export async function validateResources(sess) {
    // file+vector no projeto do work (benchmark ou prod); conversations sempre prod.
    const cli = sess.openai || openai;
    const results = await Promise.allSettled([
        cli.vectorStores.retrieve(sess.vectorStoreId),
        cli.files.retrieve(sess.openaiFileId),
        cli.conversations.retrieve(sess.conversationId_chat),
        cli.conversations.retrieve(sess.conversationId_eval),
    ]);
    const missing = [];
...
export async function rebuildSession(sess, reason) {
    ...
    // Em paralelo: re-upload dos dois PDFs (cliente do work — projeto do benchmark),
    // criar duas Conversations novas (sempre prod).
    const cli = sess.openai || openai;
    const [studentFile, enunciadoFile, chatConversation, evalConversation] = await Promise.all([
        ...
        cli.conversations.create({ metadata: { submission_token: sess.submissionToken, type: "chat", rebuilt: "1" } }),
        cli.conversations.create({ metadata: { submission_token: sess.submissionToken, type: "eval", rebuilt: "1" } }),
    ]);
    ...
    if (sess.conv_chat.length > 0) {
        try {
            await cli.conversations.items.create(sess.conversationId_chat, {
                items: sess.conv_chat.map(m => ({ role: m.role, content: m.content })),
            });
        } catch (err) {
            // Replay do chat é a MEMÓRIA da entrevista, não um detalhe (#359).
            // Falhando aqui e seguindo, o entrevistador recomeça amnésico:
            // repete perguntas já feitas, perde o fio das respostas, e a
            // entrevista resultante passa por válida na avaliação. Melhor o
            // aluno ver "tente de novo" do que fazer uma entrevista inútil.
            log.error("REBUILD", `replay chat failed: ${err.message}`);
            const wrapped = new Error(`replay_chat_failed: ${err.message}`);
            wrapped.statusCode = 503;
            wrapped.publicMessage = "Não conseguimos restaurar o histórico da sua entrevista agora. "
                + "Recarregue a página em instantes — nada do que você já respondeu foi perdido.";
            throw wrapped;
        }
    }
...
export async function createFreshSession(req) {
    ...
    // Cliente por-work: um work de benchmark cria as Conversations no SEU projeto,
    // porque o SuperOrchestrator referencia conv_chat via o parâmetro `conversation`
    // na chamada de modelo — cross-project daria 404. Ver lib/openaiClient.js.
    const cli = clientForWork(req.work);
    const [chatConversation, evalConversation] = await Promise.all([
        cli.conversations.create({ metadata: { submission_token: token, type: "chat" } }),
        cli.conversations.create({ metadata: { submission_token: token, type: "eval" } }),
    ]);
    ...
    const interviewerPersona = pickPersona({
        ...
        // Semeia por TRABALHO: mesmo work_token → mesmo entrevistador em todas
        // as entrevistas do trabalho (persona não troca de nome entre alunos).
    ...
    const sess = {
        ...
        workId: req.work.id,
        // Benchmark de custo: roteia TODAS as chamadas OpenAI deste work pela
        // chave dedicada. sess.openai flui via sessionMeterCtx → agentes/STT/TTS.
        isBenchmark: !!req.work.is_benchmark,
        openai: cli,
        submissionId: req.submission.id,
        isTest: !!req.submission.is_test,
...
export async function initOrResumeSession(req) {
    const token = req.submission.submission_token;
    // O que prova "tentativa em andamento" é o LOG DA CONVERSA, não o PDF: o
    // PDF é gravado pela prep em background antes de existir qualquer conversa,
    // e uma falha do provedor logo depois deixava a linha com PDF e nada mais.
    // Ver lib/interviewResume.js para o incidente que motivou o critério.
    ...
    const decision = resumeDecision({ hasConversationLog: !!conversationLog, runtimeRow });

    // Caso 1: nada a retomar → sessão fresca em awaiting_upload.
    if (decision === RESUME_FRESH) {
        // Runtime órfão (fase avançada sem conversa): normaliza a linha para
        // restaurar o invariante "runtime_state_json IS NULL ⇔ sem tentativa em
        // andamento". Sem isto o gate de re-envio em routes/interview.js
        // continuaria vendo uma entrevista que nunca existiu.
        if (runtimeRow) {
            log.warn("SUBMISSION", `start: runtime órfão sem conversa — limpando e recomeçando (token=${token} phase=${runtimeRow.current_phase})`);
            try { await db.clearSubmissionRuntimeState(req.submission.id); }
            catch (err) { log.error("SUBMISSION", `clear runtime órfão failed: ${err.message}`); }
        }
    ...
    // Caso 2: há conversa, mas de uma versão que não sabemos rehidratar.
    if (decision === RESUME_LEGACY) {
    ...
    // Caso 3: estado completo → rehidrata do banco.
    const sess = hydrate({
    ...
    // Cliente por-work também no caminho de retomada (hydrate não conhece o work).
    sess.isBenchmark = !!req.work.is_benchmark;
    sess.openai = clientForWork(req.work);
    ...
    // A prep ainda não persistiu file/vector store, então `sess.vectorStoreId` e
    // `sess.openaiFileId` podem estar null. NÃO validamos recursos OpenAI nesse
    // caso — `validateResources` chamaria `vectorStores.retrieve(null)`, que o SDK
    // recusa com "path inválido /vector_stores/null" (não é 404 → vira "transient"
    // → 503 num loop infinito). Em vez disso, re-disparamos a prep abaixo, que
    // recria file + vector store + plano do zero. As duas Conversations já existem
    // (criadas no /start, antes do upload) e seguem válidas — não precisam rebuild.
    const prepInFlight = sess.currentPhase === "intro" && sess.interviewPlan == null;

    // Valida recursos OpenAI em paralelo. Erro transiente → 503 (não rebuild).
    // 404 ou inativo → reconstrói. Pulado quando a prep está em vôo (acima).
    if (!prepInFlight) {
        let validation;
        try {
