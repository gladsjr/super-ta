...
// idêntico ao blob). Degradação: se o 1º pedaço falha, retorna { audio_error }
// (igual a attachAudio); falha posterior interrompe e o frontend toca o que veio.
export async function attachAudioChunks(sess, content, onChunk) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        try {
            buffer = await meteredTts(
                { ...sessionMeterCtx(sess), model: TTS_MODEL, inputText: parts[i] },
                () => synthesizeSpeech(openai, TTS_MODEL, parts[i], sess.voice)
            );
        } catch (err) {
            log.error("AUDIO", `TTS chunk ${i} failed: ${err.message}`);
            if (i === 0) return { audio_error: "tts_failed" };
            break; // toca o que já temos
        }
        const turnId = `${base}.${i}`;
        ...
        if (typeof onChunk === "function") {
            onChunk({ audio_url: url, index: i, is_last: i === parts.length - 1 });
        }
    }
    ...
    return { audio_url: firstUrl, audio_duration_seconds: totalDur || null, chunk_count: parts.length };
}
