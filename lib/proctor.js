...
const MAX_FRAMES = 7200; // DISJUNTOR (não teto de trabalho): 7200 frames = 2h a
                         // 1s/frame. Cobertura é INTEGRAL a 1 fps — amostragem
                         // adaptativa foi rejeitada (espaçar perde relance de
                         // celular; ver FRAME_SEC). O corte é feito no PRODUTOR
                         // (-frames:v) e, quando dispara, o relatório marca
                         // truncated=true — nunca corte silencioso (issue #261).
...
// duração. Antes, um vídeo de 40 min bufferizava ~2,8 GiB (+ o dobro no
// Buffer.concat) e derrubava o processo. `-frames:v` aplica o disjuntor no
// PRODUTOR: o excedente nem chega ao Node.
async function* extractFramesStream(videoPath) {
    ...
    const ff = spawn("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", videoPath, "-vf", vf, "-frames:v", String(MAX_FRAMES), "-pix_fmt", "rgb24", "-f", "rawvideo", "pipe:1"]);
    let errBuf = "";
...
export async function analyzeVideoFile(videoPath) {
    ...
    let phoneRaw = 0;
    let i = 0;
    for await (const frame of extractFramesStream(videoPath)) {
        ...
        per.push({ sec: i * FRAME_SEC, persons: det.persons, phone });
        i++;
    }
    ...
    // Persistência de "múltiplas pessoas": maior sequência CONTÍGUA (em segundos).
    // Braço/gesto do próprio aluno vira "2ª pessoa" por 1-2s; pessoa real fica.
    let multiRun = 0, run = 0;
    for (const f of per) { run = f.persons >= 2 ? run + FRAME_SEC : 0; if (run > multiRun) multiRun = run; }
    const pct = arr => Math.round((arr.length / n) * 100);
    const stamps = arr => arr.slice(0, 8).map(f => f.sec);
    const summary = {
        frames: per.length,
        interval_sec: FRAME_SEC,
        // Disjuntor de 2h disparou: só os primeiros MAX_FRAMES foram analisados.
        // O flag vai ao relatório para a tela de operações mostrar — nunca silencioso.
        truncated: per.length >= MAX_FRAMES,
        flags: {
            absent: { count: absent.length, pct: pct(absent), at: stamps(absent) },
            multiple_people: { count: multiple.length, pct: pct(multiple), at: stamps(multiple), max_run_sec: multiRun },
            phone: { count: phone.length, pct: pct(phone), at: stamps(phone), raw_count: phoneRaw },
        },
...
export async function analyzeOralVideo(videoKey) {
    ...
    try {
        ...
        log.info("PROCTOR", `analisado key=${videoKey} frames=${r.frames} ms=${r.ms} celular_bruto=${ph.raw_count ?? 0} celular_confirmado=${ph.count ?? 0}`);
        return r;
    } finally {
        fs.unlink(tmp, () => {});
    }
...
export async function analyzeOralVideoParts(videoKeys) {
    ...
    const interval = parts[0].interval_sec || FRAME_SEC;
    const mergeFlag = (name) => {
        let count = 0; const at = []; let off = 0;
        for (const p of parts) {
            const f = p.flags?.[name];
            if (f) { count += f.count || 0; for (const s of (f.at || [])) at.push(Math.round((off + s) * 10) / 10); }
            off += (p.frames || 0) * interval;
        }
        return { count, pct: Math.round((count / totalFrames) * 100), at: at.slice(0, 12) };
    };
    const summary = {
        frames: totalFrames,
        interval_sec: interval,
        parts: keys.length,
        truncated: parts.some(p => p.truncated === true),
        flags: {
