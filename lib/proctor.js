...
// Controles moles de CPU (#301, mesma receita do corte 3 de #289): todo filho
// da análise nasce via spawnLow (nice 19 + teto de threads) — a fila protege
// por concorrência, isto protege DENTRO de cada análise, para o lote nunca
// disputar de igual para igual com o relay de voz.
...
// DISJUNTOR de sanidade: 7200 quadros = 2 h a 1 fps. NÃO é orçamento a ratear —
// a amostragem é sempre densa (1 s/quadro); vídeo além de 2 h é cortado com o
// aviso `truncated` no relatório (nunca em silêncio — esse era o bug do #249).
// Histórico (#270): o teto de 1200 com intervalo ADAPTATIVO (esticado pela
// duração, #249/PR #257) existia porque a extração antiga guardava o vídeo
// inteiro em RAM. Com a extração streaming (memória constante) e a fila global
// (CPU limitada pela concorrência), rarear a densidade só piorava o recall de
// eventos breves — decisão do usuário: cobertura integral E densa.
const FRAME_BUDGET = 7200;
// `count` é em quadros e `count_sec` em segundos (iguais a 1 s/quadro); os dois
// campos seguem no relatório porque laudos antigos (era adaptativa) têm
// intervalos > 1 s e os leitores já esperam `count_sec`.
...
async function sessions() {
    ...
    // Teto de threads (#301): a inferência roda DENTRO do processo Node — nice
    // não alcança; o controle disponível é limitar o pool do onnxruntime a 1.
    // Cada análise fica ~1 CPU e o botão "análises em paralelo" vira o único
    // dial: concorrência N ≈ N CPUs de lote.
    const opt = { executionProviders: ["cpu"], graphOptimizationLevel: "all", intraOpNumThreads: 1, interOpNumThreads: 1 };
...
// Sonda do módulo NATIVO para o health check (#375, nível deep): carrega o
// onnxruntime-node, abre a sessão do detector e roda UMA inferência real num
// quadro sintético cinza 640×640. É o binário nativo — exatamente o que quebra
// ao trocar de arquitetura ou de imagem, e que hoje só se descobre na
// primeira análise de vídeo pós-prova. Lança com o motivo quando não dá.
export async function probeNativeInference() {
    const t0 = Date.now();
    const s = await sessions();
    if (!s) throw new Error("onnxruntime-node não carregou neste ambiente");
    const cinza = Buffer.alloc(IMGSZ * IMGSZ * 3, 0x72);
    const out = await infer(s.detect, frameToTensor(cinza));
    return { ms: Date.now() - t0, output_dims: Array.from(out.dims || []), model: path.basename(MODELS.detect) };
}
function iou(a, b) {
...
function probeDurationSec(videoPath) {
    return new Promise((resolve) => {
        let ff;
        try { ff = spawnLow("ffmpeg", ["-hide_banner", "-threads", "1", "-i", videoPath, "-c", "copy", "-f", "null", "-"]); }
...
//
// STREAMING (issue #261): o vídeo NÃO é carregado inteiro na memória. O stdout do
// ffmpeg é consumido por iteração assíncrona — a backpressure do pipe pausa o
// ffmpeg enquanto o consumidor infere — e cada frame é entregue e descartado
// individualmente: pico de memória ≈ 1 frame (1,2 MiB), qualquer que seja a
// duração. Antes, o buffer completo + Buffer.concat davam até ~2,8 GiB de pico e
// derrubavam o processo. `-frames:v` aplica o orçamento no PRODUTOR: o excedente
// nem chega ao Node.
async function* extractFramesStream(videoPath, intervalSec = FRAME_SEC) {
    ...
    const ff = spawnLow("ffmpeg", ["-hide_banner", "-loglevel", "error", "-threads", "1", "-i", videoPath, "-vf", vf, "-frames:v", String(FRAME_BUDGET), "-pix_fmt", "rgb24", "-f", "rawvideo", "pipe:1"]);
    let errBuf = "";
    ff.stderr.on("data", c => errBuf += c.toString());
    const exit = new Promise((resolve, reject) => {
        ff.on("error", reject);
        ff.on("close", code => code === 0 ? resolve() : reject(new Error(`ffmpeg saiu ${code}: ${errBuf.slice(0, 300)}`)));
    });
    exit.catch(() => {}); // se o consumidor abortar antes do close, não vira unhandledRejection
    const frameSize = IMGSZ * IMGSZ * 3;
    let pending = [];     // pedaços do stdout ainda não montados num frame completo
    let pendingLen = 0;
    try {
        ...
        // Consumidor pode ter abortado (erro na inferência): não deixar ffmpeg órfão.
        if (ff.exitCode === null) ff.kill("SIGKILL");
    }
    await exit; // propaga erro real do ffmpeg (código ≠ 0)
}
...
function runHandsSidecar(videoPath, signal) {
    return new Promise((resolve) => {
        if (signal?.aborted) return resolve(null);
        const py = process.env.PROCTOR_PYTHON || "python";
        const script = path.join(PROJECT_ROOT, "scripts", "proctor_hands.py");
        let pr;
        try { pr = spawnLow(py, [script, videoPath]); } catch { return resolve(null); }
        // Interrupção (#272): mata o Python; o close com code!=0 resolve null e o
        // chamador decide pelo signal.aborted que a análise toda morre.
        signal?.addEventListener("abort", () => { try { pr.kill("SIGKILL"); } catch {} }, { once: true });
        let out = "", err = "";
...
export async function analyzeVideoFile(videoPath, { signal } = {}) {
    const t0 = Date.now();
    const abortMsg = "análise interrompida pelo administrador";
    if (signal?.aborted) throw new Error(abortMsg);
    const s = await sessions();
    if (!s) throw new Error("proctoring de vídeo indisponível: onnxruntime-node não carregou neste ambiente");
    const { detect } = s;
    // Intervalo FIXO de 1 s (#270): a duração medida serve só para o relatório de
    // cobertura (covered_s / truncated) — o disjuntor FRAME_BUDGET corta em 2 h
    // com aviso, nunca em silêncio.
    const durationSec = await probeDurationSec(videoPath);
    const intervalSec = FRAME_SEC;
    const per = [];
    let phoneRaw = 0;
    let i = 0;
    for await (const frame of extractFramesStream(videoPath, intervalSec)) {
        // Aborto cooperativo (#272): lançar aqui encerra o gerador (o finally
        // dele mata o ffmpeg) — nada continua rodando em segundo plano.
        if (signal?.aborted) throw new Error(abortMsg);
        const t = frameToTensor(frame);
        const det = parseDetect(await infer(detect, t));
        let phone = false;
        if (det.phoneBoxes.length) {
            phoneRaw++;
            phone = await verifyPhoneZoom(detect, frame, det.phoneBoxes);
        }
    ...
    const side = await runHandsSidecar(videoPath, signal);
    if (signal?.aborted) throw new Error(abortMsg);
    if (side) {
...
export async function analyzeOralVideo(videoKey, { signal } = {}) {
    ...
    try {
        const r = await analyzeVideoFile(tmp, { signal });
...
export async function analyzeOralVideoParts(videoKeys, { signal } = {}) {
    const keys = (videoKeys || []).filter(Boolean);
    if (!keys.length) throw new Error("sem vídeo para analisar");
    if (keys.length === 1) return analyzeOralVideo(keys[0], { signal });
    const parts = [];
    for (const k of keys) parts.push(await analyzeOralVideo(k, { signal }));
