...
export function analyzePositionImage(imagePath) {
    return new Promise((resolve) => {
        const py = process.env.PROCTOR_PYTHON || "python";
        const script = path.join(PROJECT_ROOT, "scripts", "proctor_position.py");
        let pr;
        try { pr = spawn(py, [script, imagePath]); } catch { return resolve(null); }
        let out = "", err = "";
        pr.stdout.on("data", d => out += d);
        pr.stderr.on("data", d => err += d);
        pr.on("error", () => resolve(null));
        pr.on("close", code => {
            if (code !== 0) { log.warn("PROCTOR", `position code=${code}: ${String(err).slice(0, 150)}`); return resolve(null); }
            try { resolve(JSON.parse(String(out).trim().split("\n").filter(Boolean).pop())); } catch { resolve(null); }
        });
    });
}
