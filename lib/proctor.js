...
const IMGSZ = 640;
const DET_CONF = 0.40;   // confiança mínima (detect)
const IOU = 0.45;
const POSE_PCONF = 0.50; // confiança mínima da pessoa (pose)
const KP_CONF = 0.50;    // confiança mínima do punho p/ considerá-lo visível
...
async function sessions() {
    ...
    _sessions = {
        detect: await ort.InferenceSession.create(MODELS.detect, opt),
        pose: await ort.InferenceSession.create(MODELS.pose, opt),
    };
    return _sessions;
}
...
function parseDetect(out) {
    ...
    for (let a = 0; a < A; a++) {
        ...
        (best === 0 ? persons : phones).push({ box, score: bc });
    }
...
export async function analyzeVideoFile(videoPath) {
    const t0 = Date.now();
    const { detect, pose } = await sessions();
    const frames = await extractFrames(videoPath);
    const per = [];
    for (let i = 0; i < frames.length; i++) {
        const t = frameToTensor(frames[i]);
        const det = parseDetect(await infer(detect, t));
        const ps = parsePose(await infer(pose, t));
        per.push({ sec: i * FRAME_SEC, persons: det.persons, phone: det.phone, bothHands: ps.bothHands, personPose: ps.person });
    }
    ...
    // "mãos escondidas" só conta quando HÁ pessoa no quadro
    const handsHidden = per.filter(f => f.personPose && !f.bothHands);
    ...
    const summary = {
        ...
        flags: {
            ...
            hands_hidden: { count: handsHidden.length, pct: pct(handsHidden), at: stamps(handsHidden) },
        },
        analyzed_at: new Date().toISOString(),
        model: "yolov8n + yolov8n-pose (onnxruntime, local)",
        ms: Date.now() - t0,
    };
    return summary;
}
