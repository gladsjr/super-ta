...
export async function getStudentEvaluation(submissionId) {
    ...
    for (const s of FEEDBACK_SECTIONS) {
        ...
        if (s.key === "interviewer_opinion") {
            sectionHas[s.key] = !!opinion;
        } else {
            sectionHas[s.key] = Array.isArray(base[s.field]) && base[s.field].length > 0;
        }
    }
...
export async function setSubmissionSections(submissionId, partial) {
    ...
    const vals = [];
    for (const s of FEEDBACK_SECTIONS) {
        if (typeof partial?.[s.key] === "boolean") {
            vals.push(partial[s.key]);
            sets.push(`${s.column} = $${vals.length}`);
        }
    }
...
export async function applyWorkSectionsToAllSubmissions(workId) {
    await pool.query(
        `UPDATE submissions s SET
            include_interviewer_opinion = w.include_interviewer_opinion,
            include_strengths = w.include_strengths,
            include_improvement_areas = w.include_improvement_areas,
            include_study_suggestions = w.include_study_suggestions,
            updated_at = now()
         FROM works w
