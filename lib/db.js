...
export async function getSubmissionGrades(submissionId) {
    const r = await pool.query(
        "SELECT grades_json, grade_final::float8 AS grade_final, grades_at FROM submissions WHERE id = $1",
        [submissionId]
    );
    if (r.rowCount === 0) return null;
    ...
    if (!text) return null;
    try {
        return { ...JSON.parse(text), final: r.rows[0].grade_final, computed_at: r.rows[0].grades_at };
    } catch {
        return null;
    }
}
...
export const FEEDBACK_SECTIONS = [
    ...
    // Nota da rubrica (GradingAgent): conteúdo NÃO vive na versão do aluno — é
    // composto a partir de grades_json, como interviewer_opinion. Default
    // DESLIGADO (include_grade DEFAULT FALSE): a devolutiva só leva nota se o
    // professor optar.
    { key: "grade", column: "include_grade", field: null },
];
...
export async function getStudentEvaluation(submissionId) {
    const r = await pool.query(
        `SELECT student_evaluation_json, student_evaluation_at,
                ...
                evaluation_published_at, evaluation_json, grades_json,
                include_interviewer_opinion, include_strengths,
                include_improvement_areas, include_study_suggestions, include_grade
    ...
    const opinion = typeof internal?.interviewer_impression === "string" && internal.interviewer_impression.trim()
        ...
        : null;
    // Nota da rubrica, na forma que o ALUNO vê: nota final + critérios
    // (nome/peso/nota), SEM as justificativas (podem conter linguagem
    // forense). As justificativas ficam internas (cockpit, via getSubmissionGrades).
    const gradesRaw = parse(row.grades_json);
    const grade = gradesRaw && typeof gradesRaw.final === "number"
        ? { final: gradesRaw.final, criteria: (Array.isArray(gradesRaw.criteria) ? gradesRaw.criteria : []).map(c => ({ name: c.name, weight: c.weight, score: c.score })) }
        : null;

    const base = { ...(edited ?? auto) };
    delete base.interviewer_opinion; // a opinião nunca vive na base editável
    delete base.grade;               // a nota nunca vive na base editável

    // Flags de visibilidade (default ligado, exceto grade) e presença de
    // conteúdo por seção.
    const sections = {};
    const sectionHas = {};
    for (const s of FEEDBACK_SECTIONS) {
        ...
        else if (s.key === "grade") sectionHas[s.key] = !!grade;
        else sectionHas[s.key] = Array.isArray(base[s.field]) && base[s.field].length > 0;
    }

    // Efetiva: base sem as seções desligadas + opinião/nota injetadas se ligadas.
    const report = { ...base };
    for (const s of FEEDBACK_SECTIONS) {
    ...
    if (sections.grade && grade) report.grade = grade;
