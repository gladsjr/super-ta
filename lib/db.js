...
export async function listWorks() {
    const r = await pool.query(`
      SELECT
        ...
        -- s.id IS NOT NULL: descarta a linha-fantasma do LEFT JOIN (trabalho sem
        -- submissões), que senão cairia no ELSE 'pending' do STATUS_EXPR e
        -- inflava a contagem para "1 pendente" num trabalho vazio (#39).
...
export async function createWork(name, budgetUsd, kind = "scenario") {
    ...
    if (kind !== "interview" && kind !== "scenario") {
        throw new Error("createWork: kind must be 'interview' or 'scenario'");
    }
