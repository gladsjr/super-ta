...
export async function createWork(name, budgetUsd, kind = "interview") {
    ...
    if (kind !== "interview" && kind !== "oral_realtime") {
...
export async function setOralGrade(submissionId, grade) {
    await pool.query(
        "UPDATE submissions SET grade_final = $1, updated_at = now() WHERE id = $2",
        [grade, submissionId]
    );
}
...
export async function getOralSubmissionDetail(submissionId) {
    const r = await pool.query(
        `SELECT oral_eval_json, oral_devolutiva, oral_transcript, oral_proctor_json,
