...
export async function getStudentEvaluation(submissionId) {
    const r = await pool.query(
        `SELECT student_evaluation_json, student_evaluation_at, evaluation_published_at
         FROM submissions WHERE id = $1`,
        [submissionId]
    );
    if (r.rowCount === 0) return null;
    const row = r.rows[0];
    if (!row.student_evaluation_json) return null;
    try {
        return {
            report: JSON.parse(row.student_evaluation_json),
