...
// Configurações de devolutiva do trabalho (diretrizes + default da opinião
// do entrevistador). Valem para os lotes; ajuste por aluno fica na submissão.
export async function setWorkFeedbackSettings(workId, guidelines, includeOpinion) {
    const trimmed = typeof guidelines === "string" && guidelines.trim() ? guidelines.trim() : null;
    await pool.query(
        `UPDATE works
         SET feedback_guidelines = $1, include_interviewer_opinion = $2, updated_at = now()
         WHERE id = $3`,
        [trimmed, !!includeOpinion, workId]
    );
    return { feedback_guidelines: trimmed, include_interviewer_opinion: !!includeOpinion };
}
...
export async function createSubmissionsFromLabels(workId, labels) {
    const rows = [];
...
export async function createSubmissions(workId, baseLabel, count) {
    ...
    return createSubmissionsFromLabels(workId, labels);
}
...
// Devolve as DUAS camadas (automática + editada) e a efetiva. `report` é a
// versão EFETIVA — é o que o aluno vê e o que se publica: base = editada
// quando existir, senão automática; a OPINIÃO DO ENTREVISTADOR é a
// interviewer_impression do relatório INTERNO (evaluation_json), publicada
// como está — o professor não a edita, só decide incluir ou não
// (include_interviewer_opinion). Retorna null se nenhuma versão existe.
export async function getStudentEvaluation(submissionId) {
    const r = await pool.query(
        `SELECT student_evaluation_json, student_evaluation_at,
                ...
                evaluation_published_at, include_interviewer_opinion,
                evaluation_json
    ...
    const includeOpinion = row.include_interviewer_opinion !== false;
    ...
    const opinion = typeof internal?.interviewer_impression === "string" && internal.interviewer_impression.trim()
        ...
        : null;
    const report = { ...(edited ?? auto) };
    delete report.interviewer_opinion;
    if (includeOpinion && opinion) {
        report.interviewer_opinion = opinion;
    }
    return {
        report,
        auto_report: auto,
        ...
        include_interviewer_opinion: includeOpinion,
        has_interviewer_opinion: !!opinion,
    };
}

export async function setIncludeInterviewerOpinion(submissionId, include) {
    await pool.query(
        `UPDATE submissions
         SET include_interviewer_opinion = $1, updated_at = now()
         WHERE id = $2`,
