...
// Sem orçamento verificável e trabalho para contabilizar, nenhum check pago roda.
export function orcamentoPermite(ctx) {
    const o = ctx.orcamento;
    if (!o) return { ok: false, reason: "orçamento não resolvido; chamada paga bloqueada" };
    if (!o.work_id) return { ok: false, reason: "trabalho de saúde ausente; chamada paga bloqueada" };
...
async function checkStt(ctx = {}) {
    ...
    const r = await sttTranscribe({ openaiClient: openai, buffer, filename: `${clip}.mp3`, meterCtx: { workId: perm.workId } });
...
async function checkPythonSidecar() {
    ...
    // Prazo de 25 s (o check tem 30): o primeiro `import mediapipe` depois de
    // um Publish custou 14–15 s em prod (cache frio) e estourou os 15 s de
    // antes. Lento é aviso; só ausente ou travado é falha — como o ffmpeg.
    const py = process.env.PROCTOR_PYTHON || "python";
    const r = await comando(py, ["-c", "import mediapipe, sys; print(mediapipe.__version__)"], 25_000);
    return resultado(r.ok ? (r.ms > 10_000 ? "warn" : "ok") : "fail", { python: py, mediapipe: r.ok ? r.stdout : null, ms: r.ms, error: r.ok ? null : (r.stderr || r.error), timed_out: r.timed_out });
}
...
async function checkBudget(ctx = {}) {
    // runHealth atualiza o orçamento após os checks pagos, antes do relatório.
    const o = ctx.orcamento;
    if (!o) return resultado("fail", { reason: "orçamento não resolvido (sem leitura do banco)" });
    if (!o.work_id) return resultado("fail", { ...o, reason: "não há trabalho de saúde (works.is_health) — seedHealthWork não rodou; chamadas pagas bloqueadas" });
...
].map(c => ({ ...c, paid: ["responses", "stt", "tts"].includes(c.id), budget_ms: DEEP_BUDGET_MS }));
