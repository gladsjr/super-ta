...
export function runRetranscribeAuto({ submissionId, token, oggBuffer, tee, questions, workName, isBenchmark, meterCtx }) {
    ...
    (async () => {
        if (tee?.key) {
            try {
                const { id: jobId } = await enqueueJob("retranscribe", {
                    submissionId,
                    payload: {
...
export function runMessageRetranscribeAuto({ submissionId, token, workName, isBenchmark, meterCtx }) {
    (async () => {
        try {
            const { id: jobId } = await enqueueJob("retranscribe_message", {
                submissionId,
...
async function retranscribe({ submissionId, token, oggBuffer, tee, questions, workName, openaiClient, meterCtx, allowLocal = false }) {
    ...
    try {
        ...
        // SILÊNCIO não é falha (#344): sessão em que o aluno não chegou a falar
        // (queda logo no início, desistência antes da 1ª resposta) devolve
        // transcrição vazia. Registrar e CONCLUIR — antes o job falhava,
        // gastava as retentativas no mesmo silêncio e virava falha acionável no
        // Operações. Falha real de STT (rede, arquivo ilegível) segue subindo.
        if (!/empty transcription/i.test(String(err.message))) throw err;
        await db.setFinalTranscript(submissionId, {
            mode: "silent",
            text: "",
            note: "nenhuma fala do aluno foi captada no áudio da sessão",
            glossary_terms: keywords.length,
            quality: withCoverage({ parts: 0, suspects: 0 }, { structured: false, expectedTurns }),
            tee: teePublic(tee),
        });
        log.info("RETRANSCRIBE", `submission=${token}: sem fala do aluno (silêncio) — registrado, sem falha`);
        return;
    }
    await db.setFinalTranscript(submissionId, {
        mode: "continuous",
