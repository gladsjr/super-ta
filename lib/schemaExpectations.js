...
const listaDeColunas = (txt) => txt.split(",").map(ident).map(x => x.trim()).filter(Boolean);

// Constraint sem nome: o Postgres batiza — {tabela}_pkey, {tabela}_{cols}_key,
// {tabela}_{cols}_fkey, {tabela}_{primeira coluna da expressão}_check. Para o
// CHECK, a coluna é resolvida em expectedSchema (precisa saber as colunas da
// tabela); aqui vai a expressão. Colisão de nome (dois CHECK na mesma coluna)
// também é resolvida lá, com o sufixo numérico que o Postgres usa.
const KEYWORDS = /^(NOT|NULL|IS|IN|AND|OR|BETWEEN|TRUE|FALSE|CASE|WHEN|THEN|ELSE|END|LIKE|ILIKE|SIMILAR|TO|ANY|ALL|SOME|EXISTS|DISTINCT|FROM|ARRAY|INTERVAL|CURRENT_DATE|CURRENT_TIMESTAMP|SYMMETRIC|ASYMMETRIC|ESCAPE|COLLATE)$/i;
// Regra do Postgres (AddRelationNewConstraints): o nome de um CHECK sem nome
// leva a coluna SÓ quando a expressão referencia exatamente UMA coluna
// distinta ({t}_{col}_check); com duas ou mais, ou nenhuma, é {t}_check.
// Caso real: 063, `CHECK (delegated_packages <= granted_packages)` →
// package_allocations_check.
export function colunaDoCheck(expr, colunasDaTabela) {
    const semStrings = expr.replace(/'[^']*'/g, " ");
    const re = /("?[A-Za-z_]\w*"?)\s*(\()?/g;
    const vistas = new Set();
    let m;
    while ((m = re.exec(semStrings))) {
        if (m[2]) continue; // função
        const nome = ident(m[1]);
        if (KEYWORDS.test(nome)) continue;
        if (colunasDaTabela.has(nome)) vistas.add(nome);
    }
    return vistas.size === 1 ? [...vistas][0] : null;
}

// Uma constraint de tabela sem nome (`PRIMARY KEY (a)`, `UNIQUE (a, b)`,
// `FOREIGN KEY (a) REFERENCES ...`, `CHECK (...)`) → fato com nome derivável.
function constraintSemNome(table, clausula) {
    let k;
    if (/^PRIMARY KEY\b/i.test(clausula)) return { op: "add", kind: "constraint", table, name: `${table}_pkey`, def: clausula, auto: true };
    if ((k = /^UNIQUE\s*\(([^)]*)\)/i.exec(clausula))) return { op: "add", kind: "constraint", table, name: `${table}_${listaDeColunas(k[1]).join("_")}_key`, def: clausula, auto: true };
    if ((k = /^FOREIGN KEY\s*\(([^)]*)\)/i.exec(clausula))) return { op: "add", kind: "constraint", table, name: `${table}_${listaDeColunas(k[1]).join("_")}_fkey`, def: clausula, auto: true };
    if (/^CHECK\b/i.test(clausula)) return { op: "add", kind: "constraint", table, name: null, checkExpr: corpoParentesado(clausula, 0) || "", def: clausula, auto: true };
    return null; // EXCLUDE, LIKE: sem expectativa
}

// Uma definição de COLUNA dentro do CREATE TABLE: a coluna em si mais as
// constraints inline (nomeadas ou batizadas pelo Postgres).
function fatosDeColuna(table, clausula) {
    ...
    // Nomeadas: CONSTRAINT x <o que for>. Saem do texto para não virar automáticas.
    resto = resto.replace(/\bCONSTRAINT ("?\w+"?)\s+(PRIMARY KEY|UNIQUE|CHECK|REFERENCES|NOT NULL|NULL|DEFAULT|GENERATED)/gi, (_, nome, kw) => {
        fatos.push({ op: "add", kind: "constraint", table, name: ident(nome), def: clausula });
        return ` ${kw === "CHECK" || kw === "REFERENCES" ? "__NOMEADA__" : kw} `;
    });
    if (/\bPRIMARY KEY\b/i.test(resto)) fatos.push({ op: "add", kind: "constraint", table, name: `${table}_pkey`, def: clausula, auto: true });
    if (/\bUNIQUE\b/i.test(resto)) fatos.push({ op: "add", kind: "constraint", table, name: `${table}_${column}_key`, def: clausula, auto: true });
    if (/\bREFERENCES\b/i.test(resto)) fatos.push({ op: "add", kind: "constraint", table, name: `${table}_${column}_fkey`, def: clausula, auto: true });
    const reCheck = /\bCHECK\s*\(/gi;
    let c;
    while ((c = reCheck.exec(resto))) {
        fatos.push({ op: "add", kind: "constraint", table, name: null, checkExpr: corpoParentesado(resto, c.index) || column, def: clausula, auto: true });
    }
    return fatos;
}
...
export function factsFromStatement(stmt) {
    ...
    if ((m = /^CREATE TABLE (?:IF NOT EXISTS )?("?[\w.]+"?)/i.exec(s))) {
        ...
        // Corpo: colunas e constraints declaradas junto. `CREATE TABLE x AS
        // SELECT` não tem corpo — só a tabela é esperada.
        ...
        if (corpo) {
            for (const c of clausulas(corpo)) {
                ...
                else if (/^(PRIMARY KEY|UNIQUE|CHECK|FOREIGN KEY|EXCLUDE|LIKE)\b/i.test(c)) { const f = constraintSemNome(table, c); if (f) fatos.push(f); }
                else fatos.push(...fatosDeColuna(table, c));
            }
        }
        return fatos;
    }
    ...
    if ((m = /^ALTER TABLE (?:IF EXISTS )?(?:ONLY )?("?[\w.]+"?) (.+)$/i.exec(s))) {
        ...
        for (const c of clausulas(m[2])) {
            ...
            else if (/^ADD (PRIMARY KEY|UNIQUE|CHECK|FOREIGN KEY|EXCLUDE)\b/i.test(c)) { const f = constraintSemNome(table, c.replace(/^ADD /i, "")); if (f) fatos.push(f); }
...
export function expectedSchema(files) {
    ...
    for (const f of files) {
        ...
        for (const x of fatos) {
            ...
            else if (x.kind === "constraint" && x.op === "add") {
                let name = x.name;
                if (x.auto && name === null) {
                    // CHECK sem nome: {tabela}_{primeira coluna da expressão}_check
                    const cols = new Set([...columns.keys()].filter(k => k.startsWith(x.table + ".")).map(k => k.slice(x.table.length + 1)));
                    const col = colunaDoCheck(x.checkExpr, cols);
                    name = col ? `${x.table}_${col}_check` : `${x.table}_check`;
                }
                if (x.auto) {
                    // Colisão: o Postgres acrescenta 1, 2, ... ao nome. Nome
                    // acima do limite de 63 é truncado por regra própria —
                    // fora do escopo, sem expectativa.
                    if (name.length > 63) continue;
                    let candidato = name, n = 0;
                    while (constraints.has(`${x.table}.${candidato}`)) candidato = `${name}${++n}`;
                    name = candidato;
                }
                constraints.set(`${x.table}.${name}`, { migration: f.filename, def: x.def, auto: !!x.auto });
            }
...
export async function readCatalog(q) {
    ...
    for (const [k, sql] of Object.entries(CATALOG_SQL)) {
        ...
        // índice → tabela, para o relatório dizer o que a tabela tem
