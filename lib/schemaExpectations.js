...
function fatosDeColuna(table, clausula) {
    ...
    // PRIMARY KEY inline (`id SERIAL PRIMARY KEY`) é universal e sempre se
    // chama {t}_pkey; os outros sem nome ferem a convenção.
    ...
    if ((m = /^CREATE TABLE (?:IF NOT EXISTS )?("?[\w.]+"?)/i.exec(s))) {
        ...
        if (corpo) {
            for (const c of clausulas(corpo)) {
                ...
                else if (/^PRIMARY KEY\b/i.test(c)) fatos.push({ op: "add", kind: "constraint", table, name: `${table}_pkey`, def: c });
