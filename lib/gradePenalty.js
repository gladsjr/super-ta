...
// aluno — diferente de renderOralAlerts (factual, para a penalidade). Descreve só o
// OBSERVADO, como orientação, sem vocabulário de acusação (o StudentFeedbackAgent
// ainda passa pela varredura FORBIDDEN_PATTERNS). Null quando nada é relevante.
...
export async function applyPenaltyToGrades(work, submissionId, grades, flow, internalReport = null) {
    ...
    if (flow === "oral") {
        ...
        // Autoria (sempre) + proctoring por vídeo (opt-in) quando houver relatório.
        const lines = [renderAuthorshipAlerts(internalReport)];
        const d = await db.getOralSubmissionDetail(submissionId);
        if (d && d.oral_proctor_json) lines.push(renderOralAlerts(d.oral_proctor_json, null));
        alertsText = lines.filter(s => s && s.trim()).join("\n");
    }
