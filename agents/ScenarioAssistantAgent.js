...
- "participants": as personas desta etapa, por NOME, cada uma com um "role": ${roleList}. (Em "deliberacao" o role é ignorado.) Em CONSULTORIA/COLETA (o aluno entrevista) o role da persona é "fonte" — ela responde; quem conduz é o aluno —, NÃO "entrevista". Use "entrevista"/"questionamento" só quando a PERSONA conduz (arguição etc.).
...
- "includes_student_work": se esta etapa pede o upload do trabalho do aluno (a persona o lê). NÃO inclua este campo na resposta — deixe a FORMA decidir (apresentação/arguição/feedback/avaliação já trazem o trabalho do aluno = true; consultoria/entrevista/questionamento/deliberação = false). Só emita "includes_student_work" se o professor pedir explicitamente para mudar esse upload. Emitir "false" numa apresentação por engano DESMARCA o trabalho do aluno indevidamente — não faça isso.
- "example_questions": exemplos de falas DA PERSONA nesta etapa (inspiração para o orquestrador) — RESPEITANDO A DIREÇÃO DA FORMA. Numa etapa em que o ALUNO entrevista/pergunta à persona (consultoria/coleta, entrevista, questionamento), os exemplos são FALAS DA PERSONA respondendo/dando contexto ("Funcionamos das 6h às 20h; o pico é de manhã", "Posso te explicar; o que você quer saber primeiro?") — NUNCA as perguntas de levantamento que cabem ao ALUNO ("Como é o consumo ao longo do dia?"). Atribuir à persona as perguntas do aluno INVERTE o papel: a persona passa a entrevistar o aluno sobre dados que ela mesma deveria ter. Se a etapa é uma APRESENTAÇÃO do aluno, os exemplos são as perguntas/reações que a persona faz ao ouvi-lo. ATENÇÃO ao CLIENTE (ex.: um CFO) numa etapa em que o ALUNO o entrevista: ele RESPONDE e dá contexto (intenções, prioridades, restrições) — ele NÃO fica sondando o aluno com perguntas de decisão tipo "quais riscos você considera inaceitáveis?", "o que pesaria mais para você?", "o que você quer descobrir nos fornecedores?". Essas perguntas-da-persona-AO-aluno só cabem nas etapas de apresentação/arguição, NÃO numa entrevista/coleta.
    ...
    #cleanInteractionOp(it) {
        ...
        if (Array.isArray(it.participants)) {
            o.participants = it.participants.filter(pp => pp && this.#str(pp.persona_name)).map(pp => ({
                ...
                // Em consultoria/coleta a persona é a FONTE (o aluno conduz) — coage,
                // pois "conduz a entrevista" ali contradiz a forma. Ver defaultRoleForForm.
                role: o.form === "consultoria" ? "fonte" : (PARTICIPANT_ROLES.includes(pp.role) ? pp.role : "questionamento"),
            }));
        }
