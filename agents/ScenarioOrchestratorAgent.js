...
- FORA DO ESCOPO: action.hint {title, body} é EXCLUSIVO para quando a outra ponta insiste num tópico que está EXPLICITAMENTE listado numa seção "FORA DO ESCOPO" do briefing. Pré-condições (TODAS obrigatórias): (a) o briefing TEM uma seção "FORA DO ESCOPO"; e (b) o tópico da fala bate com um item dela. Se NÃO existe seção "FORA DO ESCOPO" no briefing, NUNCA use action.hint — não há nada fora do escopo. E JAMAIS marque como fora do escopo uma pergunta sobre o que a persona sabe, sobre a informação privada dela, ou sobre o próprio tema da etapa: perguntas difíceis sobre limitações, custo, prazo, riscos e trade-offs são DENTRO do escopo — responda/desconverse no papel, SEM hint. Quando o hint for cabível (raro): responda EVASIVO/reconduza no papel SEM dizer "fora do escopo" na cena, e preencha action.hint em meta. Omita action.hint em todo o resto.
...
${PERSONA_EXCHANGE_SCHEMA_DESCRIPTION}`;
    ...
    async _call(systemPrompt, userContent, { meterCtx, vectorStoreIds = [], label, onFirstDelta = null, onMessageReady = null, onMessageDelta = null, onSpeaker = null }) {
        const payload = {
            model: this.model,
                ...
                if (wantStream) {
                    ...
                    for await (const event of stream) {
                        if (event?.type === "response.output_text.delta") {
                            ...
                            // speaker_persona_id é o 1º campo do JSON — sai antes da message;
                            // avisamos cedo para a bolha em streaming já nascer com o nome certo.
                            if (typeof onSpeaker === "function" && !speakerSignaled) {
                                const s = extractJsonStringValue(buf, "speaker_persona_id");
                                if (s && s.complete && s.value.trim()) { speakerSignaled = true; try { onSpeaker(s.value.trim()); } catch (e) { log.error(label, `onSpeaker: ${e.message}`); } }
                            }
    ...
    async studentTurn({
        ...
        onFirstDelta = null, onMessageReady = null, onMessageDelta = null, onSpeaker = null,
    }) {
...
Escolha quem fala e a próxima ação. Retorne SOMENTE o JSON do schema.`;
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
            const text = await this._call(systemPrompt, userContent, { meterCtx, vectorStoreIds, label: "AGENT:ScenarioOrchestrator", onFirstDelta: attempt === 1 ? onFirstDelta : null, onMessageReady: attempt === 1 ? onMessageReady : null, onMessageDelta: attempt === 1 ? onMessageDelta : null, onSpeaker: attempt === 1 ? onSpeaker : null });
