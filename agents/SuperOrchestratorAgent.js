...
- "follow_up": pedir complemento sobre o turno ATUAL quando a resposta tem incoerência relevante, está incompleta em relação ao escopo da pergunta, OU contradiz algo verificável (ver bloco VERIFICAÇÃO DE CONTRADIÇÕES abaixo). ATENÇÃO ao PRINCÍPIO DA RESPOSTA FORMULÁVEL DE CABEÇA (acima): numa sondagem quantitativa, uma resposta qualitativa — direção + mecanismo + ordem de grandeza — NÃO é "incompleta"; aceite-a e siga, NÃO dispare follow_up só porque falta o valor exato recalculado. SEMPRE acompanhe de follow_up_reason — escolha o valor que mais se aplica: "incoherence" | "incomplete" | "diminishing_returns" | "contradicts_work" | "contradicts_earlier_self". ORÇAMENTO DE INSISTÊNCIA POR PERGUNTA (o contexto do turno informa o saldo): follow_up por "incomplete" ou "incoherence" — NO MÁXIMO 1 POR PERGUNTA; se após essa única tentativa a resposta continuar vaga ou incompleta, registre a lacuna em memory.open_threads e SIGA para a próxima ask, aceitando a resposta imperfeita (a lacuna registrada JÁ É o achado — a avaliação a verá). follow_up por "contradicts_work"/"contradicts_earlier_self" — até 2 consecutivos (contradição concreta merece tentativa de reconciliação). EM PARTICULAR: cobrança de fonte/referência/base numérica que já foi feita UMA vez e não foi respondida NUNCA deve ser repetida em outras palavras — registre e avance. A pressão é sobre o conteúdo, no registro da persona — não sobre a pessoa.
...
${resolvedBody}`;
        ...
        // Saldo do orçamento de insistência da pergunta corrente (regra no
        // systemPrompt): 1 follow_up p/ incomplete/incoherence; 2 p/ contradições.
        ...
        const budgetLine = `Orçamento de insistência NESTA pergunta: incomplete/incoherence ${softUsed}/1${softUsed >= 1 ? " — ESGOTADO (PROIBIDO follow_up por completude: registre a pendência em memory.open_threads e avance)" : ""}; contradições ${contraUsed}/2${contraUsed >= 2 ? " — ESGOTADO (registre o conflito em open_threads e avance)" : ""}`;
